...
2. agent_notes/ns_genus_c_recursion.tex, especially the fixed-parity
...
Only algebraic identities are tested here.  Convention phases are kept
explicit rather than silently folded into an eta or spin-lift variable.
"""
...
def base_fixed_parity(a: int, b: int, c: int, h1: sp.Expr, h2: sp.Expr, h3: sp.Expr) -> sp.Expr:
    """The fixed-parity zero-translation table in SCblock (3.4)--(3.5)."""
    table = {
        (0, 0, 0): sp.S.One,
        (1, 0, 0): sp.S.One,
        (0, 1, 0): sp.S.One,
...
def rho_collaborator(
    ...
    """Collaborator's complete vertex: explicit kernels plus a derivative."""
    H1 = h1 + sp.Rational(a, 2)
    H2 = h2 + sp.Rational(b, 2)
    H3 = h3 + sp.Rational(c, 2)
    exponent = H1 + k1 - H2 - H3 - k3
    return sp.expand(t_collaborator(k1, k3, a, b, c, h1, h2, h3) * falling(exponent, k2))
...
def check_shadow_torus_one_point() -> None:
    ...
    h, d = sp.symbols("h d")
    bottom_ratio = base_fixed_parity(1, 0, 1, h, d, h) / (2 * h)
    upper_ratio = base_fixed_parity(1, 1, 1, h, d, h) / (2 * h)
    assert_zero("torus B0 odd/even magnitude", bottom_ratio - (2 * h - d) / (2 * h))
    assert_zero(
        "torus B1 odd/even magnitude",
        upper_ratio - (2 * h + d - sp.Rational(1, 2)) / (2 * h),
    )
    # The shadow paper uses a relative minus between the two sl(2) sectors.
    sewing_phase = -1
    paper_b0_second = -(2 * h - d) / (2 * h)
    paper_b1_second = -(2 * h + d - sp.Rational(1, 2)) / (2 * h)
    assert_zero("torus B0 shadow sign", sewing_phase * bottom_ratio - paper_b0_second)
    assert_zero(
        "torus B1 shadow sign",
        sewing_phase * upper_ratio - paper_b1_second,
    )
...
def check_shadow_component_relations() -> None:
    ...
    assert_zero(
        "shadow relation 101",
        base_fixed_parity(1, 0, 1, D1, h, D2) - (D1 + D2 - h),
    )
    assert_zero(
        "shadow relation 011",
        base_fixed_parity(0, 1, 1, D1, h, D2) - (D1 - h - D2),
    )
    assert_zero(
        "shadow relation 110",
        base_fixed_parity(1, 1, 0, D1, h, D2) - (D1 + h - D2),
    )
    # Their component-labelled matrix element has the opposite 111 sign.
    # Keep that local convention phase explicit; it cancels or combines with
    # the supertrace phase in the block-level comparisons above.
    assert_zero(
        "shadow relation 111 convention phase",
        -base_fixed_parity(1, 1, 1, D1, h, D2)
