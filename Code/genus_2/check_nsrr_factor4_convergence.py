...
def target_design():
    ...
    return dict(config=config, method='refine_nsrr_bilinear_quadrature.target_node; N=3, R=16',
        implementation_sha256={str(p.relative_to(ROOT)): previous.sha(p)
        for p in (ROOT/'Code/genus_2/refine_nsrr_bilinear_quadrature.py',
...
def report():
    ...
    target = matrix(last['target_density'])
    baseline = inputs.read(QUAD/'order-N7.json')
    frame = baseline['free_frame_power']
    geometry_config = inputs.read(QUAD/'config.json')
    point = geometry_config['point']; b = geometry_config['b']
    assert abs(frame/(point['source_free']/point['target_free'])**(1+2*(b+1/b)**2)-1) < 1e-14
    history = []
    for n in range(3, 11):
    ...
    assert nodes[0]['design_digest'] == object_digest(target_design())
    ...
    validation = inputs.read(OUT/'bank_validation.json')
    assert validation['status'] == 'passed'
    for node in validation['nodes']:
        for path, digest in node['input_sha256'].items():
            assert hashlib.sha256(inputs.bytes(ROOT/path)).hexdigest() == digest
    tests = inputs.read(OUT/'tests.json')
    ...
    result = dict(status='complete_conditional_normalization_test', **POLICY, b=1.4, t=.6,
        ...
        saved_bank_validation={k:v for k,v in validation.items() if k not in ('nodes','implementation_sha256')},
        unit_tests=tests,
    ...
    for path in (Path(__file__), ROOT/'Code/genus_2/nsrr_normalization.py', ROOT/'Code/genus_2/nsrr_bilinear_sewing.py',
                 ...
                 ROOT/'Code/genus_2/audit_nsrr_factor4_saved_banks.py', ROOT/'Code/genus_2/run_nsrr_resummed_frozen_grid.py'):
        inputs.bytes(path)
...
def write_report(result, banks):
    ...
    lines += ['', '## Saved block-order convergence', '',
        ...
        'L is total descendant level; R is the target recursion twice-level parameter.', '',
    ...
    for level in LEVELS:
        ...
        delta='—' if level==LEVELS[0] else f'{max(r["absolute_nodal_relative_step"] for r in rows.values()):.3e}'
        lines.append(f'| {level} | {rows["s=+1"]["ratio"]:.10f} | {rows["s=-1"]["ratio"]:.10f} | {rows["spin_sum"]["ratio"]:.10f} | {delta} |')
    ...
    validation=result['saved_bank_validation']
    lines += ['', '### Saved-data checks', '',
        'Three existing panel-grid nodes (head, bulk and tail) were recomputed through L8 with the current native '
        'engine, evaluating all four equal-eta channels directly, including f=1 without parity reconstruction. '
        f'The largest scaled coefficient discrepancy is `{validation["maximum_scaled_coefficient_error"]:.3e}`; '
        f'the projected complex blocks agree at saved floating-point precision. The supplied BRY constants agree '
        f'with the independent 40-digit evaluator to `{validation["maximum_relative_BRY_error"]:.3e}` relative. '
        'This is a selected-node validation, not a fresh recomputation of all 2071 bank nodes.', '',
        f'{result["unit_tests"]["test_count"]} tests pass, covering independent complex anti data, all tube signs, '
        'the normalization at the resummed API boundary, and existing local BPZ/primary bookkeeping.', '',
        'Across all five saved surfaces on the complete panel N12 source grid, the largest absolute nodal '
        'L7-to-L8 change divided by Z is '
        f'`{max(max(r["absolute_nodal_relative_steps"].values()) for r in banks["rows"] if r["grid"]=="panel_N12" and r["L"]==8):.3e}`. '
        'Only the central surface has the refined target quadrature used below.']
