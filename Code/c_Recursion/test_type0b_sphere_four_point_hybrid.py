...
class Type0BSphereFourPointHybridTests(unittest.TestCase):
    def test_default_c_recursion_in_every_frame_and_region(self):
        from run_type0b_sphere_four_point_analytic_hybrid import _routing_audit
        ...
        for z in (0.001 + 0.001j, 0.5 + 0.3j, 0.999 + 0.001j):
            audit = _routing_audit(kernel, z, tuple(range(6)))
            self.assertTrue(audit["passed"])
            self.assertEqual(audit["policy"], "all-c")
            self.assertFalse(audit["h_recursion_enabled"])
            for record in audit["frames"]:
                channel = four_point_channel_from_ordering(
                    kernel.fixed_positions(z), record["ordering"]
                )
                for region in ("auto", "bulk", "corner"):
                    with self.subTest(z=z, frame=record["frame"], region=region):
                        self.assertEqual(kernel._selected_backend(channel, region), "c")

        with mock.patch.object(kernel, "_selected_backend", return_value="h"):
            with self.assertRaisesRegex(ArithmeticError, "requires c-recursion"):
                _routing_audit(kernel, 0.5 + 0.3j, (0,))
