...
class GenusTwoNSPartitionTests(unittest.TestCase):
    ...
    def test_bry_odd_coefficient_is_rephased_at_human_note_boundary(self) -> None:
        momenta = (0.37, 0.61, 0.83)
        precision = 35
        even = ns_structure_constant(*momenta, precision=precision)
        odd = ns_tilde_structure_constant(*momenta, precision=precision)
        self.assertAlmostEqual(
            _structure_weight("theta", 0, momenta, precision),
            (even * even).real,
            places=13,
        )
        self.assertAlmostEqual(
            _structure_weight("theta", 1, momenta, precision),
            -(odd * odd).real,
            places=13,
        )

        left, right, bridge = momenta
        odd_left = ns_tilde_structure_constant(
            left, bridge, left, precision=precision
        )
        odd_right = ns_tilde_structure_constant(
            right, bridge, right, precision=precision
        )
        self.assertAlmostEqual(
            _structure_weight("glasses", 1, momenta, precision),
            -(odd_left * odd_right).real,
            places=13,
        )
    ...
    def test_functional_recursion_preserves_order_twelve_benchmarks(self) -> None:
        ...
        expected = {
            ...
            "glasses": {
                # PBW-verified glasses convention: ordinary large-c
                # vacuum/global product plus incidence-ordered self-loop
                # residues.
    ...
    def test_cannon_shard_identity_validation(self) -> None:
                    ...
                    "sectors": [
                        {
                            ...
                            "absolute_three_form_parity": 0,
                            "partition_sign": 1,
                            "contribution": 0.0,
                        },
                        {
                            "sector": 1,
                            "antiholomorphic_sector": 1,
                            "absolute_three_form_parity": 1,
                            "partition_sign": -1,
                            "contribution": 0.0,
                        },
                    ],
        ...
        wrong_sign = deepcopy(shard)
        wrong_sign["radius_results"][0]["sectors"][1]["partition_sign"] = 1
        with self.assertRaisesRegex(RuntimeError, "partition sign mismatch"):
            _validate_shard(config, 0, wrong_sign, "review-fingerprint")
