#!/usr/bin/env python3
"""Target-blind analytic-continuation pilot with all-c recursion.
...
Every crossed Liouville pole is included. The moduli integrand uses
component-aware c-recursion in every selected channel, at every nome.
The historical script name is retained for compatibility.
...
def _routing_audit(
    ...
    passed = bool(records) and all(record["backend"] == "c" for record in records)
    if not passed:
        raise ArithmeticError("amplitude routing requires c-recursion in every frame")
    return {
        "z": _pair(z),
        "frames_checked": list(frames),
        "policy": "all-c",
        "h_recursion_enabled": False,
        "passed": passed,
        "frames": records,
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload: dict[str, object] = {
        ...
        "recursion_atlas": {
            "block_backend": "c",
            "h_rule": "disabled for amplitude runs",
            "c_rule": "channel-adapted c recursion in every frame at every nome",
