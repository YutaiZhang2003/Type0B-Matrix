...
For the even HJS three-form labelled by ``sign=+/-1``, the primary ground
matrix is therefore ``diag(1, sign*kappa_left*kappa_right)``.  The Ward
recursion below is the R-R form in section 2 of Suchanek,
...
class DirectNRRThetaOracle:
    ...
    def coefficient(self, ns_twice_level: int, r1_level: int, r2_level: int) -> complex:
        ...
        for parity_1 in (0, 1):
            ...
            plumbing_lift = (
                self.lifts[0] ** ns_parity
                * self.lifts[1] ** parity_1
                * self.lifts[2] ** parity_2
            )
            total += (
                theta_orientation_sign((parity_1, ns_parity, parity_2))
                * plumbing_lift
                * contracted
            )
        return complex(total / self.normalization)
