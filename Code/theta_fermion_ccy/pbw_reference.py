"""Independent physical PBW computation, authorized through total level ten.
...
def encode(value, machine=False):
    if machine:
        value = complex(value)
        if not math.isfinite(value.real) or not math.isfinite(value.imag):
            raise ArithmeticError("Nonfinite native PBW coefficient")
...
def main():
    ...
    parser.add_argument("--machine", action="store_true",
                        help="Use native binary64 scalars and complex128 Gram matrices")
    parser.add_argument("--json", type=Path, required=True)
    args = parser.parse_args()
    if not 0 <= args.level <= 10:
        parser.error("The requested PBW computation is limited to total level 10")

    if args.machine:
        from check_level10_machine import load_runner
    else:
    ...
    report = {
        ...
        "etas": [args.eta, -args.eta], "precision_bits": 53 if args.machine else 384,
        "numerical_backend": "native complex128" if args.machine else "FLINT complex midpoints",
        "coefficients": [],
    }
    ...
    for levels in runner.level_triples(2*args.level):
        ...
        report["coefficients"].append({
            "twice_levels": levels, "values": [encode(x,args.machine) for x in coefficient]})
