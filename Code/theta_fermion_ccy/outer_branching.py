...
class _MPLowAnchors(direct.DirectBranchingCoefficient):
    ...
    def __init__(self, b, momenta, primary_parity, dps, precision_bits=None):
    ...
    def raw(self, labels, alpha2, alpha3, eta):
        ...
        previous = br.MP_DPS
        previous_bits = mp.mp.prec
        br.set_multiprecision(self.dps, precision_bits=self.precision_bits)
        try:
            return super().raw(labels,alpha2,alpha3,eta)
        finally:
            br.set_multiprecision(previous, precision_bits=previous_bits if previous else None)
...
class _MPBranchingGrid(BranchingGrid):
    def __init__(self, b, momenta, cutoff, primary_parity=0, mp_dps=60, precision_bits=None):
        ...
        br.set_multiprecision(self.mp_dps, precision_bits=self.precision_bits)
    ...
    def build_actions(self):
        ...
        self.r_action_providers = {
            slot: RamondActions(self.b, self.momenta[slot],
                                precompute_plus=getattr(self, "middle_plus_labels", ()) if slot == 1 else ())
            for slot in (1, 2)
        }
        for slot, provider in self.r_action_providers.items():
            for label in self.r:
                ...
                self.r_actions[slot, 1, label] = provider.minus(label, 1)
                for parity in (0, 1):
                    self.action_diagnostics.append({
...
class OuterBranching:
    def __init__(self, b, momenta, level, *, f=0, p=0, dps=0, machine=False,
                 precision_bits=None, precompute_middle=False):
        started = time.perf_counter()
        ...
        self.grid.middle_plus_labels = (frozenset(n for pair in pairs for n in pair
                                                 if abs(n) > Fraction(1, 4))
                                        if precompute_middle else frozenset())
        ...
        self.diagnostics = {
            ...
            "ward_precision_bits": mp.mp.prec if dps else 53,
    ...
    def middle_actions(self, dps, precision_bits=None):
        if self.grid.mp_dps != dps or not hasattr(self.grid,"r_action_providers"):
            return None
        if getattr(self.grid,"precision_bits",None) != precision_bits:
            return None
