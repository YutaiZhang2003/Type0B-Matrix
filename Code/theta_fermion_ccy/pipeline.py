...
from schottky_vacuum import theta_seed_provenance
from series_algebra import (ZERO, scalar_product, recover_minus, theta_sign,
                            diagonal_virasoro_indices, diagonal_virasoro_product)
...
def set_arithmetic(dps, precision_bits=None):
    global ARITHMETIC
    ARITHMETIC = mp.mp if dps else mp.fp
    if dps:
...
def numeric(value):
    if isinstance(value, Fraction):
        return ARITHMETIC.mpf(value.numerator)/value.denominator
    return ARITHMETIC.mpc(value)


def encoded(value):
    value = numeric(value)
    if ARITHMETIC is mp.fp:
        return {"real": repr(value.real), "imag": repr(value.imag)}
...
def multiply_scalar_parity(scalar, parity, cutoff):
    ...
    for a, factor in scalar.items():
        for b, vector in parity.items():
            ...
            previous = answer.setdefault(key, [ARITHMETIC.mpc(0)]*8)
...
def reduced_numerator(b, momenta, level, *, f=0, p=0, etas=(1,-1),
                      dps=60, ward_dps=0, progress=None, branching_checkpoint=None,
                      precision_bits=None, diagonal_only=True, ccy_method="forward"):
    if ccy_method not in ("forward","backward"):
        raise ValueError('ccy_method must be forward or backward')
    engine_class = ForwardPuncturedCCY if ccy_method == 'forward' else PuncturedCCY
    started = time.perf_counter()
    if progress:
        progress({"stage": "outer branching", "status": "starting", "level": level})
    outer = OuterBranching(b, momenta, level, f=f, p=p, dps=ward_dps,
                           machine=(dps == 0), precision_bits=precision_bits,
                           precompute_middle=(ward_dps == dps))
    outer.prepare(etas)
    outer_seconds = time.perf_counter()-started
    if progress:
        progress({"stage": "outer branching", "status": "complete", "seconds": outer_seconds})
    # Restore the requested CCY context after the separate outer Ward solve.
    # Convert its benchmark weights directly from the rational inputs.
    set_arithmetic(dps, precision_bits=precision_bits)
    b_mp = numeric(b)
    momenta_mp = tuple(map(numeric, momenta))
    shared_actions = outer.middle_actions(dps, precision_bits=precision_bits)
    middle = MiddleBranching(b_mp, momenta_mp[1], dps=dps, actions=shared_actions,
    ...
    answer, ccy_records = {}, []
    cases = 0
    transposed_products = 0
    ccy_seconds = 0.0
    virasoro_seconds = 0.0
    virasoro_product_seconds = 0.0
    last_progress = time.perf_counter()
    pairs = middle_pairs(level)
    tick = time.perf_counter()
    middle_values = {(outgoing,incoming,alpha): numeric(middle.raw(outgoing,incoming,alpha))
                     for incoming,outgoing in pairs for alpha in (0,1)}
    middle_seconds = time.perf_counter()-tick
    if progress:
        progress({"stage":"middle branching", "seconds":middle_seconds,
                  "coefficients_stored":len(middle_values),
                  "outer_actions_reused":shared_actions is not None})
    checkpoint_started = time.perf_counter()
    if branching_checkpoint:
        branching_checkpoint({
            "outer_tables":[{"alpha":alpha,"gamma":gamma,"eta":eta,
                "coefficients":[{"labels":list(map(str,labels)),"value":encoded(value)}
                                for labels,value in sorted(values.items())]}
                for (alpha,gamma,eta),values in sorted(outer.tables.items())],
            "middle_coefficients":[{"outgoing":str(outgoing),"incoming":str(incoming),
                                    "parity":alpha,"value":encoded(value)}
                for (outgoing,incoming,alpha),value in sorted(middle_values.items())],
            "outer_seconds":outer_seconds,"middle_seconds":middle_seconds,
            "outer_diagnostics":outer.diagnostics,
            "outer_actions_reused_by_middle":shared_actions is not None,
        })
    branching_checkpoint_seconds = time.perf_counter()-checkpoint_started
    r3_labels = tuple(n for n in outer.grid.r if ramond_level(n) <= level)
    for n1 in ns_labels(level):
        # Exchanging the two identical outer Virasoro vertices transposes
        # the middle matrix element and the two split propagation powers.
        # Reuse only this scalar product; each orientation retains its own
        # branching prefactors, norms and parity contribution below.
        products = {}
        for incoming,outgoing in pairs:
            for n3 in r3_labels:
                ...
                if remaining < 0:
                    continue
                if diagonal_only:
                    budget = (2*level-shift[0]-shift[3])//2
                    left_budget, right_budget = budget-shift[1], budget-shift[2]
                    if min(left_budget,right_budget) < 0:
                        continue
                factors = []
                for alpha in (0,1):
                    gamma = (f-int(2*n1)-alpha) % 2
                    inserted = middle_values[outgoing,incoming,alpha]
                ...
                if not any(value for _,value in factors):
                    continue
                tick = time.perf_counter()
                pair_key = (min(incoming,outgoing),max(incoming,outgoing),n3)
                if pair_key in products:
                    product_started = time.perf_counter()
                    previous_product = products.pop(pair_key)
                    product = {(a,r,l,d):value for (a,l,r,d),value in previous_product.items()}
                    transposed_products += 1
                    virasoro_product_seconds += time.perf_counter()-product_started
                else:
                    indices = (diagonal_virasoro_indices(left_budget,right_budget) if diagonal_only
                               else downward_indices(remaining,degree_weights=(2,1,1,2)))
                    ...
                    blocks = []
                    for copy in (0,1):
                        virasoro_started = time.perf_counter()
                        engine = engine_class(central[copy],
                                             tuple(pair[copy] for pair in edge_weights),
                                             external[copy],dps=dps,precision_bits=precision_bits)
                        blocks.append(engine.reduced_series(indices=indices))
                        ccy_records.append({"labels":list(map(str,(n1,incoming,outgoing,n3))),
                                            "copy":copy+1,"cache":engine.cache_info()})
                        engine.clear_caches()
                        virasoro_seconds += time.perf_counter()-virasoro_started
                    product_started = time.perf_counter()
                    product = (diagonal_virasoro_product(blocks[0],blocks[1],left_budget,right_budget)
                               if diagonal_only else scalar_product(lift_scalar(blocks[0]),lift_scalar(blocks[1]),remaining))
                    products[pair_key] = product
                    virasoro_product_seconds += time.perf_counter()-product_started
    ...
    diagnostics = {"outer":outer.diagnostics,"outer_seconds":outer_seconds,
                   ...
                   "virasoro_recursion_seconds":virasoro_seconds,
                   "virasoro_product_seconds":virasoro_product_seconds,
                   "branching_checkpoint_seconds":branching_checkpoint_seconds,
                   "middle_coefficients_stored":len(middle_values),
                   "outer_actions_reused_by_middle":shared_actions is not None,
                   "assembly_precision_bits":ARITHMETIC.prec,
                   "diagonal_only":diagonal_only,"ccy_method":ccy_method,
                   "branch_cases":cases,"ccy":ccy_records,
                   "ccy_series_computed":len(ccy_records),
                   "transposed_products_reused":transposed_products,
                   "middle_recursion_count":len(middle.recursion_records)}
    diagnostics["numerator_total_seconds"] = time.perf_counter()-started
...
def main():
    ...
    parser.add_argument("--precision-bits",type=int,
                        help="Use this exact bit precision for every numerical stage, overriding both decimal precision settings")
    parser.add_argument("--machine", action="store_true",
                        help="Use native binary64 for branching, CCY and recovery; exact auxiliary and Schottky series remain exact")
    parser.add_argument("--sector-policy", choices=("error","record"), default="error",
                        help="Record continues through numerical sector residuals, retaining all coefficients unchanged")
    ...
    parser.add_argument("--ccy-method",choices=("forward","backward"),default="forward",
                        help="Forward reuses null-shift weights across every final coefficient")
    parser.add_argument("--full-split",action="store_true",
                        help="Retain the former full four-variable numerator and quotient")
    ...
    if args.precision_bits is not None:
        if args.machine or args.auxiliary != "direct":
            parser.error("--precision-bits requires the multiprecision direct-fermion pipeline")
        if args.precision_bits < 103:
            parser.error("Use at least 103 bits (30 decimal digits), or select --machine")
        mp.mp.prec = args.precision_bits
        args.dps = args.ward_dps = mp.mp.dps
    if args.machine:
        if args.auxiliary != "direct":
            parser.error("The native binary64 pipeline requires the direct fermion backend")
        args.dps = args.ward_dps = 0
    elif args.dps < 30 or (args.ward_dps and args.ward_dps < 30):
        parser.error("Use at least 30 decimal digits, or select --machine")
    ...
    set_arithmetic(args.dps, precision_bits=args.precision_bits)
    b = Fraction(7,5)
    ...
    report = {"status":"running", "total_q_level":args.level,
              ...
              "requested_precision_bits":args.precision_bits,
              "ccy_precision_bits":ARITHMETIC.prec,
              "numerical_backend": "native binary64" if args.machine else "mpmath",
              "ccy_method":args.ccy_method,"diagonal_only":not args.full_split,
              "sector_policy":args.sector_policy,
              "source_hashes":source_hashes()}
    args.json.parent.mkdir(parents=True,exist_ok=True)
    report["reduced_numerator"],diagnostics = None,None
    branching_path = args.json.with_name(args.json.stem+"_branching.json")
    report["branching_file"] = str(branching_path)
    def progress(value):
        print(json.dumps(value),flush=True)
    def save_branching(values):
        saved = {"status":"branching coefficients computed",
                 "parameters":{key:report[key] for key in ("total_q_level","b","momenta","p","f","etas","ccy_dps","ward_dps","requested_precision_bits")},
                 "source_hashes":report["source_hashes"], **values}
        branching_path.write_text(json.dumps(saved,indent=2,default=str)+"\n")
    numerator,diagnostics = reduced_numerator(
        ...
        branching_checkpoint=save_branching,precision_bits=args.precision_bits,
        diagonal_only=not args.full_split,ccy_method=args.ccy_method)
    ...
    if args.auxiliary == "direct":
        ...
        auxiliary = auxiliary_engine.series(args.level, progress=progress, diagonal_only=not args.full_split)
        ...
        if not args.full_split:
            auxiliary = {key:value for key,value in auxiliary.items() if key[1]==key[2]}
    ...
    args.json.write_text(json.dumps(report,indent=2,default=str)+"\n")
    tick = time.perf_counter()
    report["recovery_diagnostics"] = {}
    try:
        quotient = recover_minus(numerator,auxiliary,2*args.level,tolerance=1e-8,
                                 sector_policy=args.sector_policy,
                                 diagnostics=report["recovery_diagnostics"],diagonal_only=not args.full_split)
    except ArithmeticError as exc:
        report["convolution_division_seconds"] = time.perf_counter()-tick
    ...
    vacuum = theta_vacuum_seed(args.level)
    report["vacuum_seed_seconds"] = time.perf_counter()-seed_started
    report["vacuum_seed_provenance"] = theta_seed_provenance(args.level)
    restoration_started = time.perf_counter()
    seed = {(2*a,b,b,2*c):numeric(value) for (a,b,c),value in vacuum.items()}
    if seed_power == 2:
    ...
    report["vacuum_restoration_seconds"] = time.perf_counter()-restoration_started
