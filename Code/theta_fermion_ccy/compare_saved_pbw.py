...
def main():
    ...
    parser.add_argument("--comparison-dps",type=int,default=70)
    parser.add_argument("--json",type=Path,required=True)
    args = parser.parse_args()
    started = time.perf_counter()
    mp.mp.dps = args.comparison_dps
    ...
    for level in range(args.level+1):
        ...
        cumulative.append({
            "through_total_level":level,**stats(subset),
            "equal_split_powers":stats([r for r in subset if r[2]["exponents"][1] == r[2]["exponents"][2]]),
            "unequal_split_powers":stats([r for r in subset if r[2]["exponents"][1] != r[2]["exponents"][2]]),
        })
    report = {
        ...
        "pbw_arithmetic":pbw.get("numerical_backend","FLINT complex midpoints, inherited scalar clipping below 1e-80"),
        "production_arithmetic":production.get("numerical_backend","unspecified"),
        "comparison_arithmetic_dps":mp.mp.dps,
        "comparison_arithmetic_scope":"Subtraction and error statistics for saved coefficient strings only; neither block is recomputed",
