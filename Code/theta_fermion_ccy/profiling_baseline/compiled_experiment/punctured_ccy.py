...
class PuncturedCCY:
    ...
    def __init__(self, central_charge, weights, external_weight, dps=80, precision_bits=None,
                 native_recursion=None):
        self._use_native = dps == 0 and native_recursion is not False
        self._native_stats = None
        ...
        # Native entries are small; this capacity retains the transitions of
        # a maximum-cutoff N=10 branch instead of repeatedly evicting them.
        # Keep the previous bound for multiprecision scalar objects.
        transition_capacity = 32768 if dps else 131072
        self._transition = lru_cache(maxsize=transition_capacity)(self._transition_indexed)
    ...
    def _transition_indexed(self,c_id,shifts,edge,r,s):
        transition = self._transition_uncached(c_id,_vectors[shifts],edge,r,s)
        if transition is None:
            return None
        pole_id,factor,shifted = transition
        return pole_id,factor,_vector_id(shifted)

    def _coefficient_uncached(self,c_id,shifts,levels):
        total = self._global(shifts,levels)
        transition_at = self._transition
        coefficient_at = self._coefficient
        global_at = self._global
        for edge,r,s,lower,terminal in _level_steps_indexed(levels):
            transition = transition_at(c_id,shifts,edge,r,s)
            if transition is None:
                continue
            pole_id,factor,shifted = transition
            # Terminal levels have no admissible r*s residue, independently
            # of c. The immutable level stencil is shared across all branches.
            coefficient = (global_at(shifted,lower) if terminal else
                           coefficient_at(pole_id,shifted,lower))
    ...
    def reduced_series(self,cutoff=None,*,indices=None,degree_weights=(1,1,1,1)):
        ...
        if self._use_native:
            from native_recursion import load_native, native_info
            native = load_native()
            self._native_provenance = native_info()
            if native is not None:
                indices = tuple(tuple(int(n) for n in level) for level in indices)
                if any(len(level) != 4 or min(level) < 0 for level in indices):
                    raise ValueError("four nonnegative descendant levels are required")
                levels = tuple(_vector_id(level) for level in indices)
                values, self._native_stats = native.evaluate(
                    self._global_indexed, self._transition_indexed,
                    _level_steps_indexed, levels)
    ...
    def cache_info(self):
        ...
        info["recursion_backend"] = "compiled loop, CPython arithmetic" if self._native_stats is not None else "Python"
        info["native_provenance"] = self._native_provenance
        if self._native_stats is not None:
            info.update(self._native_stats)
        return info
