...
class RamondActions:
    ...
    def module(self, reflected=False):
        return CachedActionModule("R", self.b,
    ...
    def plus(self, label, parity):
        ...
        terms, fit = solve_ramond_lplus(module, n, parity)
        self.diagnostics[("plus", label, parity)] = {
            key: value for key, value in fit.items() if key != "coefficients"
        }
        return tuple(br.ActionTerm(sign*term.label, term.first, term.second, term.coefficient)
                     for term in terms)
    ...
    def minus(self, label, parity):
        label = _label(label)
        sign = 1 if label > 0 else -1
        terms, fit = solve_ramond_lminus(self.module(sign < 0), abs(label), parity)
