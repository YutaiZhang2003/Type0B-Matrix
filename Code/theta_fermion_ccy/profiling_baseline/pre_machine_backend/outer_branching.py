...
class _SparseWardFactorization:
    ...
    def __init__(self, rows, column_count, dps):
        ...
        squared_norms = [mp.mpf(0) for _ in range(column_count)]
        for row in rows:
            for column, value in row.items():
                squared_norms[column] += abs(value)**2
        self.norms = tuple(mp.sqrt(value) for value in squared_norms)
...
class _MPBranchingGrid(BranchingGrid):
    ...
    def _prepare_system(self, alpha2, alpha3, form_parity):
        ...
        for labels in unknowns:
            ...
            norm = mp.sqrt(mp.fsum(abs(value)**2 for value in equation.values()))
            if norm > mp.power(10, -max(20, self.mp_dps-20)):
        ...
        rows.extend({index[labels]: mp.mpc(1)} for labels in anchors)
...
class OuterBranching:
    def __init__(self, b, momenta, level, *, f=0, p=0, dps=0):
        ...
        self.diagnostics = {
            ...
            "closed_triples": len(self.grid.required) if dps else None,
