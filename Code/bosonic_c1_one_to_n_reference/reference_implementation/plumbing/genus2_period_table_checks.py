...
def check_forward_glass_grid() -> None:
    config_path = (
        Path(__file__).resolve().parent
        / "config"
        / "genus2_glass_forward_period_table_cluster.json"
    )
    config = load_config(config_path)
    require(
        nominal_point_count(config) == 122_880,
        "forward glass candidate count changed",
    )
    description = describe(config)
    require(
        description["design_mode"] == "glass-forward-q-grid",
        "forward glass design mode was lost",
    )
    first = next(iter_manifest_rows(config))
    require(first["topology"] == "glasses", "forward grid emitted a non-glass row")
    require("sobol_index" in first, "forward grid lost Sobol provenance")
    require("atlas_target_index" not in first, "forward grid incorrectly depends on Omega targets")
    require("omega11" not in first, "forward grid preparation evaluated a period matrix")
