...
def check_retry_manifest_and_overlay() -> None:
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        require(
            summary["retry_period_tolerance"] == 1.0e-9,
            "retry manifest did not record the relaxed tolerance",
        )
        preflight = retry_preflight(retry_manifest, payload)
        require(bool(preflight["retry_ready"]), "retry preflight did not pass")
        require(
            preflight["retry_period_tolerance_counts"] == {"1.0000000000000001e-09": 2},
            "retry preflight did not preserve the per-row tolerance",
        )
