...
The explicit fixed-puncture trace and maximal theta-graph audit separately
certify the remaining BRST period-form bridge.
"""
...
try:
    ...
    from genus2_integrand_normalization import (
        ...
        GENUS1_FIXED_PUNCTURE_BRST_TRACE_CERTIFIED,
        ...
        RAW_THETA_PRODUCT,
        MAXIMAL_FULL_BRST_FACTORIZATION_CERTIFIED,
        MAXIMAL_FULL_BRST_SEWING_INTERNAL_CONSISTENCY_CHECK_PASSED,
...
def check_polyakov_gauge_fixing_recurrence() -> None:
    ...
    c1_topology_metric = c1_residual_topology_metric(alpha_prime)
    ...
    print(f"  fixed K_res,S2 c=1           = {c1_topology_metric:.16e}")
    ...
    print(f"  current c=1 topology/form mult= {c1_kernel_multiplier:.16e}")
    _require(
    ...
    _require(
        ...
        "the energy-delta c=1 sphere metric is not 4*pi",
    )
    _require(
        _relative_error(c1_topology_metric, 2.0) < 2.0e-15,
        "the residual compact c=1 topology metric is not two",
    )
    _require(
    ...
    _require(
        _relative_error(
            genus2_topology_correction,
            4.0 * math.pi / alpha_prime,
        )
        < 2.0e-15,
        "the c=1 genus-two topology correction is not 4*pi/alpha'",
    )
    _require(
        _relative_error(
            c1_kernel_multiplier,
            4.0 * GENUS2_BRST_PERIOD_TO_PUNCTURED_SEWING_MULTIPLIER,
        )
        < 2.0e-15,
        "the completed c=1 BRST kernel multiplier is not 64*pi^4",
    )
    _require(
        WORLDSHEET_GAUGE_FIXING_NORMALIZATION_CERTIFIED,
    ...
    _require(
        ...
        and MAXIMAL_FULL_BRST_SEWING_INTERNAL_CONSISTENCY_CHECK_PASSED
        and not MAXIMAL_FULL_BRST_FACTORIZATION_CERTIFIED
        and not GENUS2_C1_ABSOLUTE_NORMALIZATION_CERTIFIED
        and not FULL_CFT_FACTORIZATION_CERTIFIED,
        "the working production coefficient must remain numerically checked "
        "but explicitly uncertified pending the critical-string theta control",
    )
...
def check_mqm_dictionary() -> None:
    ...
    print(f"  unit-Mumford domain target R=1 = {normalized_integral:.16e}")
    ...
    _require(
        ...
        < 2.0e-15,
        "the self-dual bare period/Mumford integral target is incorrect",
    )
    _require(
        _relative_error(
            GENUS2_FUNDAMENTAL_DOMAIN_WEIGHT * coarse_integral,
            normalized_integral,
        )
        < 2.0e-15,
        "the genus-two fundamental-domain target changed under Sp/PSp notation",
    )
    _require(
        _relative_error(
        ...
        < 2.0e-15,
        "the string-note domain integral does not reproduce the MQM coefficient",
    )
    _require(
        _relative_error(
            unconverted_bry_over_xi,
        ...
        < 2.0e-15,
        "the bare BRY topology ledger must remain separate from the "
        "fixed-puncture BRST period-form conversion",
    )
    _require(
    ...
    _require(
        _relative_error(
            GENUS2_FUNDAMENTAL_DOMAIN_WEIGHT * coarse_string_note_integral,
            string_note_integral,
        )
        < 2.0e-15,
        "the string-note target changes under Sp/PSp notation",
    )
    _require(
