...
def _hybrid_config(
    ...
    *,
    ...
    period_tolerance_override: float | None = None,
) -> HybridPeriodMapConfig:
    numerics = payload["numerics"]  # type: ignore[index]
    domain = payload["q_domain"]  # type: ignore[index]
    return HybridPeriodMapConfig(
        tolerance=(
            float(period_tolerance_override)
            if period_tolerance_override is not None
            else float(numerics["period_tolerance"])
        ),
...
def evaluate_manifest_row(
    ...
    *,
    ...
    retry_tolerance_text = str(row.get("retry_period_tolerance", "")).strip()
    retry_tolerance = float(retry_tolerance_text) if retry_tolerance_text else None
    configured_basis = int(payload["numerics"]["maximum_collocation_basis"])  # type: ignore[index]
    configured_tolerance = float(payload["numerics"]["period_tolerance"])  # type: ignore[index]
    ...
    if retry_tolerance is not None and (
        not math.isfinite(retry_tolerance)
        or retry_tolerance < configured_tolerance
    ):
        raise ValueError("retry period tolerance must be finite and no stricter than production")
    config = _hybrid_config(
        payload,
        maximum_basis_override=retry_basis,
        period_tolerance_override=retry_tolerance,
    )
...
def preflight(manifest: Path, payload: dict[str, object]) -> dict[str, object]:
    expected_digest = config_sha256(payload)
    mode = str(payload["useful_region"]["mode"])  # type: ignore[index]
    ...
    q_grid_provenance_complete = True
    q_min = math.inf
    q_max = 0.0
    minimum_geometry_margin = math.inf
    ...
    for row in _manifest_rows(manifest):
        ...
        if not all(
            row.get(key, "").strip()
            for key in ("sobol_index", "pattern_index", "design_seed")
        ):
            q_grid_provenance_complete = False
        routes[f"{row['planned_backend']}:{row['precision_tier']}"] += 1
        topologies[str(row["topology"])] += 1
        strata[str(row["stratum"])] += 1
        shards[int(row["shard_id"])] += 1
        q_min = min(q_min, float(row["q_min"]))
        q_max = max(q_max, float(row["q_max"]))
        minimum_geometry_margin = min(
            minimum_geometry_margin, float(row["geometry_margin"])
        )
    ...
    result: dict[str, object] = {
        ...
        "topology_row_counts": dict(topologies),
        "stratum_row_counts": dict(strata),
        "planned_crosschecks": crosschecks,
        "q_abs_observed_min": q_min,
        "q_abs_observed_max": q_max,
        ...
        result.update(
            {
                ...
                "production_ready": len(shards) == expected_shards
                and mp_ready
                and selector_ready,
            }
        )
    elif mode == FORWARD_GRID_MODE:
        domain = payload["q_domain"]  # type: ignore[index]
        minimum_rows = int(payload["grid_design"]["minimum_manifest_rows"])  # type: ignore[index]
        grid_ready = bool(
            payload["useful_region"]["status"] == "forward-grid-approved-for-manifest"  # type: ignore[index]
            and q_grid_provenance_complete
            and topologies == Counter({"glasses": rows})
            and rows >= minimum_rows
            and q_min >= float(domain["q_abs_tail_min"]) * (1.0 - 1.0e-12)
            and q_max <= float(domain["q_abs_max"]) * (1.0 + 1.0e-12)
            and minimum_geometry_margin >= float(domain["geometry_margin_min"])
        )
        result.update(
            {
                "q_grid_provenance_complete": q_grid_provenance_complete,
                "minimum_required_rows": minimum_rows,
                "forward_grid_ready": grid_ready,
                "production_ready": len(shards) == expected_shards
                and mp_ready
                and grid_ready,
            }
        )
    else:  # pragma: no cover - configuration validation excludes this
...
def build_retry_manifest(
    *,
    ...
    payload: dict[str, object],
    maximum_basis: int,
    period_tolerance: float | None = None,
) -> dict[str, object]:
    ...
    configured_tolerance = float(payload["numerics"]["period_tolerance"])  # type: ignore[index]
    if period_tolerance is not None and (
        not math.isfinite(float(period_tolerance))
        or float(period_tolerance) < configured_tolerance
    ):
        raise ValueError("retry period tolerance must be finite and no stricter than production")
    ...
    with manifest.open(newline="") as source, temporary.open("w", newline="") as target:
        ...
        for row in reader:
            ...
            row["retry_period_tolerance"] = (
                "" if period_tolerance is None else f"{float(period_tolerance):.17g}"
            )
...
def retry_preflight(manifest: Path, payload: dict[str, object]) -> dict[str, object]:
    ...
    tolerances: Counter[float] = Counter()
    source_statuses: Counter[str] = Counter()
    for row in _manifest_rows(manifest):
        row_id = row["row_id"]
        ...
        shards[int(row["shard_id"])] += 1
        ceilings[ceiling] += 1
        tolerance_text = str(row.get("retry_period_tolerance", "")).strip()
        tolerance = (
            float(tolerance_text)
            if tolerance_text
            else float(payload["numerics"]["period_tolerance"])  # type: ignore[index]
        )
        if not math.isfinite(tolerance) or tolerance < float(
            payload["numerics"]["period_tolerance"]  # type: ignore[index]
        ):
            raise ValueError(f"retry row {row_id} has an invalid period tolerance")
        tolerances[tolerance] += 1
    ...
    return {
        ...
        "retry_period_tolerance_counts": {
            f"{key:.17g}": value for key, value in tolerances.items()
        },
        "source_status_counts": dict(source_statuses),
...
def run(argv: Sequence[str] | None = None) -> None:
    ...
    retry_manifest_parser.add_argument("--period-tolerance", type=float)
