...
auxiliary ground vector is singular in four star-spectral channels. There
is no inverse on the entire parity algebra, but equal-structure physical
blocks lie in its nonsingular ideal. ``recover_same_structure_nsrr_series``
performs coefficientwise division in that ideal. Opposite-structure blocks
are annihilated by this auxiliary factor and are explicitly excluded. See
``full_ramond_block_runtime/PHYSICAL_RECOVERY.md`` for the parity identity and
the restricted inverse. The independent physical
...
def ramond_sector_projection(
    vector: Sequence[complex], sector: int = 1
) -> ParityVector:
    """Project onto (1 + sector * eta_R1 eta_R2)/2 in the star algebra."""

    if sector not in (-1, 1):
        raise ValueError("sector must be +1 or -1")
    if len(vector) != 8:
        raise ValueError("the theta parity algebra has eight components")
    # x star e_p = (-1)^(p_R1+p_R2) e_(p xor 6), where x=eta_R1 eta_R2.
    return tuple(
        (
            complex(vector[index])
            + sector
            * (-1) ** (((index >> 1) & 1) + ((index >> 2) & 1))
            * complex(vector[index ^ 6])
        ) / 2
        for index in range(8)
    )


def ramond_sector_residual(
    series: Mapping[Level, Sequence[complex]], sector: int = 1
) -> float:
    """Largest coefficientwise scaled distance from the selected ideal."""

    return max(
        (
            max(abs(a - b) for a, b in zip(vector, ramond_sector_projection(vector, sector)))
            / max(1.0, *(abs(value) for value in vector))
            for vector in series.values()
        ),
        default=0.0,
    )


def recover_same_structure_nsrr_series(
    enlarged: Mapping[Level, Sequence[complex]],
    auxiliary: Mapping[Level, Sequence[complex]],
    *,
    maximum_total_twice_level: int,
    etas: Sequence[int] = (1, 1),
    sector_tolerance: float = 1.0e-8,
) -> ComponentSeries:
    """Recover physical F^(eta,eta) without using physical PBW coefficients.

    Ramond parity exchange gives x star F^(eta,eta') = eta*eta' F,
    x=eta_R1 eta_R2. For equal vertex signs, F is in the ideal with unit
    e_+=(1+x)/2. The auxiliary constant is 2 e_+, so ascending q-coefficients
    satisfy F_N=(Fhat_N-sum_{0<M<=N} auxiliary_M star F_(N-M))/2.

    Inputs must include the constant term and every coefficient through the
    requested cutoff (omitted entries mean zero). Roundoff within the stated
    tolerance is projected onto the ideal; larger violations are rejected.
    Opposite vertex signs require independent enlarged data and cannot be
    recovered by this function. ``etas`` are vertex-form signs, not tube signs.
    """

    cutoff = int(maximum_total_twice_level)
    if cutoff < 0 or cutoff != maximum_total_twice_level:
        raise ValueError("the twice-level cutoff must be a nonnegative integer")
    if len(etas) != 2 or any(eta not in (-1, 1) for eta in etas):
        raise ValueError("etas must contain two vertex signs, each +1 or -1")
    if etas[0] != etas[1]:
        raise ValueError("opposite vertex signs are lost by this auxiliary factor")
    if not np.isfinite(sector_tolerance) or sector_tolerance <= 0:
        raise ValueError("sector_tolerance must be finite and positive")

    def prepare(series, name):
        selected = {}
        for levels, vector in series.items():
            if len(levels) != 3 or any(int(level) != level or level < 0 for level in levels):
                raise ValueError(f"{name}: levels must be three nonnegative integers")
            if levels[1] % 2 or levels[2] % 2:
                raise ValueError(f"{name}: Ramond twice-levels must be even")
            if sum(levels) > cutoff:
                continue
            values = tuple(complex(value) for value in vector)
            if len(values) != 8 or not all(np.isfinite(value) for value in values):
                raise ValueError(f"{name}: coefficients must be eight finite numbers")
            selected[tuple(int(level) for level in levels)] = values
        residual = ramond_sector_residual(selected)
        if residual > sector_tolerance:
            raise ValueError(
                f"{name} violates the recoverable Ramond sector: "
                f"scaled residual {residual:.3e} exceeds {sector_tolerance:.3e}"
            )
        return {levels: ramond_sector_projection(vector) for levels, vector in selected.items()}

    numerator = prepare(enlarged, "enlarged block")
    denominator = prepare(auxiliary, "auxiliary block")
    ground = denominator.get((0, 0, 0), ZERO_VECTOR)
    expected_ground = (1, 0, 0, 0, 0, 0, 1, 0)
    if max(abs(a - b) for a, b in zip(ground, expected_ground)) > sector_tolerance:
        raise ValueError("the auxiliary constant must be 1 + eta_R1 eta_R2")

    positive_terms = [
        (levels, vector)
        for levels, vector in denominator.items()
        if sum(levels) > 0 and any(vector)
    ]
    answer: ComponentSeries = {}
    for levels in level_triples(cutoff):
        residual = list(numerator.get(levels, ZERO_VECTOR))
        for aux_levels, aux_vector in positive_terms:
            remaining = tuple(levels[edge] - aux_levels[edge] for edge in range(3))
            if min(remaining) < 0:
                continue
            product = star_multiply(aux_vector, answer.get(remaining, ZERO_VECTOR))
            residual = [a - b for a, b in zip(residual, product)]
        answer[levels] = tuple(value / 2 for value in residual)
    return answer
