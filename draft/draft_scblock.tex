...
\section{Decomposing superconformal blocks}
...
\fe
One can define the convolution inverse $\mathbf{F}_{\mathsf{F}}^{-1}$ of the free fermion block by $\mathbf{F}_{\mathsf{F}}^{-1}\star \mathbf{F}_{\mathsf{F}}= 1$, and the superconformal block can be written as
\ie \label{convo}
\mathbf{F}_f^{(\eta, \eta')}(h_1, \beta_2, \beta_3) = \mathbf{F}_{\mathsf{F}}^{-1} \star \widehat{\mathbf{F}}_f^{(\eta, \eta')}(h_1, \beta_2, \beta_3).\fe
...
On the other hand, one can choose the basis $L_{-A}^{(1)}L_{-B}^{(2)}v_n$ for $\mathcal{M}_{\NS}$ and $L_{-A}^{(1)}L_{-B}^{(2)}v_n^\alpha$ for $\mathcal{M}_{\R}$, which leads to the following decomposition of the enlarged block:
\begin{widetext}
\ie
...
\fe
Once one knows the branching coefficients, one can use the $c$-recursion relation to compute the Virasoro conformal blocks \cite{Cho:2017oxl}, which computes both $\mathbf{F}_{\mathsf{F}}$ and $\widehat{\mathbf{F}}_f^{(\eta, \eta')}(h_1, \beta_2, \beta_3)$ as a $q$-series. One can then use \eqref{convo} to compute the superconformal block.
...
We are grateful to Ben Mazel, Victor A. Rodriguez, and Jaroslav Scheinpflug for discussions. This work is supported by DOE grant DE-SC0007870. YW and XY thank the Yukawa Institute for Theoretical Physics for its hospitality during the course of this work. ChatGPT 5.6 Sol was used in this work for drafting and editing, literature searches, algorithm implementation, and conducting numerical tests. 
