...
class DirectNSSpherePBW:
    """Full plane comb coefficients for any number of bottom NS fields."""

    def __init__(self, c, external, internal, symbolic=False):
    ...
    def propagated(self, levels):
        ...
        if edge == 0:
            vector = self.matrix([self.left.value(state,0,()) for state in states])
        else:
            previous = self.propagated(levels[:-1])
            previous_states = basis(levels[-2])
            vector = self.matrix([
                sum(self.middle[edge-1].value(bra,0,ket)*previous[j] for j,ket in enumerate(previous_states))
                for bra in states
            ])
    ...
    def coefficient(self, levels):
        ...
        result = sum(self.right.value((),0,state)*propagated[i] for i,state in enumerate(basis(levels[-1])))
