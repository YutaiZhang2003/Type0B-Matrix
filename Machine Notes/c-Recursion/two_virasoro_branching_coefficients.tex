...
Here we translate the free-field prescription of arXiv:1111.2803 into the
notation and ordered three-point convention of the human note.  Writing
\(k=2n\in\mathbb Z\), the vector is a finite Fermi-sea product of the modes
\(\chi_r=f_r-i\vartheta_r\).  The paper gives a factorized blow-up ratio
\(l\), and its identity specialization gives the Shapovalov norm.  A separate
step is required to identify the paper's ordered matrix element with the
fixed-parity trilinear form \(\widehat\rho_a\) of the human note.  We keep
these two quantities distinct below and state explicitly what has and has not
been checked.  No Whittaker vector is used.
\end{abstract}
...
where
\begin{equation}
 T_a^{\mathrm H}:=\hatrho_a(v_{1,n_1},v_{2,n_2},v_{3,n_3}),
 \qquad
