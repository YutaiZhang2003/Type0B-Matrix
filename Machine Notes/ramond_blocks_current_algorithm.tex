...
second choice at $N=10$, including the corner $q_1^{10}q_2^{10}q_3^{10}$.
...
\input{ramond_blocks_current_algorithm_per_edge_timings.tex}
...
The cost model uses saved physical-block logs and a short arithmetic-only
calibration of their backend at 136 and 384 bits. The sign choice does not change
...
\input{ramond_blocks_current_algorithm_per_edge_pbw.tex}
...
All reference physical blocks were already stored; no new physical PBW
block was computed. Regenerate the timing and PBW tables from saved records,
