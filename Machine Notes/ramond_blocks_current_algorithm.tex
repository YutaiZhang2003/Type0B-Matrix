...
data; solve the outer Ward systems and, for the insertion, the middle
recurrence; evaluate the reduced Virasoro factors at branch-dependent cutoffs;
...
\path{Human Notes/SCblock.tex}, with the contour transport specified below.
...
collisions, and singular Ward pivots. A limiting value at such a point
requires a separate meromorphic-continuation prescription.
...
words; $\alpha$ is the physical second-slot ground parity:
\begin{align}
 \hat\rho_f^{(\eta)}=
 &(-i)^{\#G(A)\bmod2}
 (-1)^{f(\#G(A)+\#G(B)+\alpha)+\#G(A)\#\mathsf A}\nonumber\\
 &\times(-1)^{(\#G(B)+\alpha+p_1)(\#\mathsf C+\mathsf c)}
...
the two $(-i)^{\#G(A)\bmod2}$ factors cancel the physical NS contour sign.
...
\subsection{Outer Ward system and low anchors}
The physical $L_1$ identity gives
\begin{align}
 ...
 \label{eq:outer-ward}
\end{align}
Insert these action expansions and factor each descendant form into its primary
coefficient and two ordinary Virasoro three-point factors. This is a linear
system for the raw $\hat\rho_f^{(\eta)}$ at fixed $f,\alpha,\gamma$.
Start with the candidate primary triples for the branch sum, then close this set
under every action-label change in \eqref{eq:outer-ward}.
Keep only $2n_1+\alpha+\gamma\equiv f\pmod2$ in each system.
This avoids solving on an unnecessarily large rectangular grid.
The action stage still prepares the edge-label ranges before this closure.
For the inserted graph, its initial candidate set uses the average of the
two middle primary levels; the stricter maximum-level condition in
Section~\ref{sec:truncation} is imposed when assembling the diagonal.
Thus support closure and target truncation reduce different stages.

The system is anchored at $n_1=0,\pm1/2$ and
$n_2,n_3=\pm1/4,\pm3/4$. Their explicit free-field states are converted
to small physical PBW states, and their physical and auxiliary three-point
forms are computed by Ward identities with \eqref{eq:vertex-frame}.
Only these anchors and the low boundary conversion use physical PBW;
no high-level physical block or high-level Gram sewing is part of production.
The matrix is independent of $\eta$ and $p_1$; its right-hand side contains
the anchor dependence. At fixed run parameters one factorization is reused
for both $\eta$ signs. The ordinary pipeline requests only its one sign.
...
are cached in the outer system.
...
\item \textbf{Scaled finite systems.} Columns are normalized once. Native
action solves use full-column-rank LAPACK least squares. Outer systems and
multiprecision action systems use pivoted QR of the transposed scaled matrix
to select independent rows, then an LU factorization of that square restriction.
The factorization is reused for each applicable right-hand side.
...
\begin{equation}
 ...
 \sum_{c_*}w_{\boldsymbol s}(c_*)
 \frac{R_{rs}^{(e)}(\boldsymbol h+\boldsymbol s)}
      {c_*-c_{rs}(h_e+s_e)}.
 \label{eq:forward}
\end{equation}
Keep $c_*$ separate during propagation: summing it away would lose the
different outgoing denominators. After outgoing transitions have been processed,
...
The cutoff is simply
$|\mathsf A|+|\mathsf B|+|\mathsf C|\le N$.
...
solving a coefficient. Updates stop at the cutoff and never revisit a solved
...
column-rank selection and LU preconditioning are still double precision.
...
Native action residuals are required below $10^{-8}$; selected native Ward
refinement targets $10^{-12}$ and native sparse pruning uses $10^{-13}$.
...
\begin{equation}
 ...
 \quad(\eta,\eta')=(+,+)\text{ or }(+,-).
\end{equation}
Each level and sector is a separate process with empty numerical caches.
They run sequentially; within-process reuse remains enabled.
...
\path{C++/results/current_algorithm_2026-09-11/}.

\input{ramond_blocks_current_algorithm_timings.tex}

\subsection{PBW workload estimate, without running PBW}
PBW times below refer to the existing physical SCA implementation in Python,
with its numerical backend stated explicitly; they are not estimates for a
hypothetical optimized C++ PBW code. The dimensions used for work counts are
\begin{equation}
...
Costs are calibrated against saved logs only. The sign choice does not change
basis sizes; cached reuse of the same three-point form for identical vertices
must be accounted for
separately when comparing the two sectors. A fitted range measures sensitivity
to the cost model, not a confidence interval or a guaranteed runtime bound.

\input{ramond_blocks_current_algorithm_pbw.tex}
...
Ward values are reused within a run. Pole, fusion, and global caches belong
...
\begin{verbatim}
make -C 'C++' -j2
python3 'C++/tools/time_current_pipelines.py' --dps 40 \
  --output 'C++/results/current_algorithm_2026-09-11'
\end{verbatim}
The coefficient convention in JSON is $[a,l,l,d]$ for
$q_1^{a/2}q_2^lq_3^{d/2}$, with $a+2l+d\le2N$ and even $d$.
Parity slot is $\epsilon_1+2\epsilon_2+4\epsilon_3$.
Before these timing runs, the C++ migration compared all 728 physical slots
through level 5 against saved Python results. Maximum scaled differences were
$1.21\times10^{-11}$ (ordinary machine), $1.59\times10^{-10}$ (inserted machine),
$1.98\times10^{-25}$ (ordinary 40 digits), and $3.45\times10^{-25}$
(inserted 40 digits). These are numerical comparisons, not error bounds.
The present task runs no new physical PBW calculation and no unrequested
expensive numerical cross-check.
