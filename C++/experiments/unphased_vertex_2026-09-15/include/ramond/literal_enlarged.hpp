...
namespace ramond {
template<class S> class LiteralEnlargedPBW {
    ScaWords<S> words_;
  ...
  public:
    ...
    LiteralEnlargedPBW(S b,const std::array<S,3> &p,int primary_parity,int form_parity,int eta,bool opposite)
    ...
    // Literal PBW sum with the unphased human SCA vertex, raw Koszul
    // reordering, and (-1)^mathsf{A}; no (-1)^physical{A} is present.
    // For the inserted block the equal-level middle contraction is Theta psi_0.
    std::array<S,8> coefficient(std::array<int,3> total2) {
        std::array<S,8> result{};
        for(int pa=0;pa<=total2[0];pa++)
        for(int pb=0;pb<=total2[1];pb+=2)
        for(int pc=0;pc<=total2[2];pc+=2) {
            const int aa=total2[0]-pa, ab=(total2[1]-pb)/2, ac=(total2[2]-pc)/2;
            for(int parity=0;parity<2;parity++) {
                ...
                for(const auto &B:partitions(ab,true,false))
                for(const auto &C:partitions(ac,true,false))
                for(int bg=0;bg<2;bg++) {
                    int cg=(A.size()+B.size()+C.size()+bg)%2;
                    AuxTriple auxstates{AuxState{A,0},AuxState{B,bg},AuxState{C,cg}};
                    S aux=fermion_form_.complex_value<S>(auxstates);
                    if(aux==S(0)) continue;
                    int ap=A.size()%2, bp=(B.size()+bg)%2, cp=(C.size()+cg)%2;
                    int koszul=((pr[0]+p_)%2)*(bp+cp)+pr[1]*cp;
                    std::vector<S> vertices[2];
                    for(int v=0;v<(opposite_?2:1);v++) {
                        vertices[v].reserve(volume);
                        for(const auto &x:physical[v]) vertices[v].push_back(S(sign(koszul))*aux*x);
                    }
                    int index=((pr[0]+p_+ap)%2)|((pr[1]^bp)<<1)|((pr[2]^cp)<<2);
                    S sewing=S(sign(ap)*theta_sign(index));
                    if(opposite_) sewing*=insertion_scalar_*S(sign(bg));
                    result[index]+=sewing*contract(vertices[0],edges,vertices[opposite_?1:0]);
                }
            }
        }
        return result;
