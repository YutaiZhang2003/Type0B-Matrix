...
static void help() {
    std::cout
        << "Usage: ramond --mode ordinary|inserted --level N --dps 0|DIGITS --json FILE\n"
           "  --vertex transported|unphased|production (default: transported)\n"
           "  --ns-branch-sign keep|remove (default: keep); directed level <=3 probe.\n"
...
int main(int argc, char **argv) {
    try {
        ...
        for (int i = 1; i < argc; i++) {
            ...
            if (key == "--vertex") {
                require(value == "unphased" || value == "production" || value == "transported", "invalid vertex");
                unphased_vertex = value != "production";
                apply_vertex_transport = value == "transported";
            } else if (key == "--ns-branch-sign") {
                require(value == "keep" || value == "remove", "invalid NS branch sign");
                keep_ns_branch_sign = value == "keep";
            } else if (key == "--mode") {
        ...
        require(s.level<=3,"this directed probe is limited to level <=3");
        ...
        auto write = [&](const auto &result) {
            ...
            std::ostringstream encoded; encode_result(encoded, s, result);
            out << "{\"experimental_vertex\":\"" << (unphased_vertex?(apply_vertex_transport?"transported":"unphased"):"production")
                << "\",\"ns_branch_sign\":\"" << (keep_ns_branch_sign?"keep":"remove")
                << "\"," << encoded.str().substr(1);
            out.close();
