...
def main():
    ...
    for primary,f,eta,mode in product(range(2),range(2),[1,-1],['ordinary','inserted']):
        ...
        checks.extend(local)
        (ROOT/'theta_partial.json').write_text(json.dumps(dict(runs=records,comparisons=checks),indent=2)+'\n')
    ...
    sources += [ROOT/name for name in ['theta_validate.cpp','theta_tests.py','theta_makefile']]
    hashes={str(path.relative_to(CPP)):hashlib.sha256(path.read_bytes()).hexdigest() for path in sources}
    summary=dict(scope='theta NS-R-R, NS primary parity p=0,1, total level <=6; f=0,1, both eta signs and both eta*eta-prime signs; all 8 parity slots and all 8 plumbing spin evaluations',
