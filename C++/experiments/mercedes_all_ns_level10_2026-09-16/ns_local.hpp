...
template<class S> class NSBranches {
    ...
    const std::vector<Term>&branch(int slot,int n){
        ...
        double branch_started=seconds();if(std::abs(n)>=8)std::cerr<<"NS branch state slot="<<slot<<" label="<<n<<" start\n";
        ...
        if(std::abs(n)>=8)std::cerr<<"NS branch state slot="<<slot<<" label="<<n<<" terms="<<out.size()<<" seconds="<<seconds()-branch_started<<"\n";
