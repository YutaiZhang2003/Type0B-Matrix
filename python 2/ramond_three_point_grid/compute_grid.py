#!/usr/bin/env python3
"""Historical direct NS--R--R branching grid (even-primary convention).

This file is retained only to reproduce the earlier grid data.  New Ramond
PBW/Ward computations must use
``Code/PBW_c_recursion_double_virasoro crosscheck/ramond_pbw_generalized_ward.py``,
where ``p_phi`` is mandatory and the generalized Ward sign is explicit.
...
class PhysicalNRREvaluator:
    """Legacy even-primary evaluator retained for historical grid replay."""
...
def ns_components(branch_label, q_value, momentum):
    ...
    for physical_count in range(count + 1):
        for physical_modes2 in combinations(all_modes2, physical_count):
            ...
            ordered_basis, coefficients = ns_branch.abstract_eta_coefficients_at(
                physical_modes2, q_value, momentum
            )
