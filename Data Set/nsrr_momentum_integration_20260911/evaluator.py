...
class Evaluator:
    def __init__(self, point_ids=None, *, reference_constants=False, native_dps=40):
        ...
        self.native_dps=int(native_dps)
        if self.native_dps not in (0,40):raise ValueError('supported native precisions are 0 and 40')
    ...
    def source(self, momenta, levels=(1,3)):
        ...
        if self.native_dps==0 and max(levels)>3:
            raise ValueError('machine precision is restricted to the low-order integration control')
        def compute(dps):
            return trial.block_components(b,momenta[::-1],max(levels),native_dps=dps)
        try:
            components,checks=compute(self.native_dps)
            residual=max(c['diagnostics'][key] for c in checks['native_channels']
                         for key in ('maximum_action_residual','maximum_ward_residual','maximum_sector_residual'))
            if self.native_dps==0 and residual>1e-11:
                components,checks=compute(40)
        except ArithmeticError:
            if self.native_dps!=0:raise
            components,checks=compute(40)
        checks['requested_control_precision']=self.native_dps
        checks['multiprecision_fallback']=self.native_dps==0 and checks['native_precision_digits']==40
        rows=[]
