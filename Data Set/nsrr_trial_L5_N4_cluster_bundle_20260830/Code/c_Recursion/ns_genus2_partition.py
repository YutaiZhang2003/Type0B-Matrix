...
* the theta nonchiral sum uses the human-note factor
  ``(-1)^(a+p1+p2+p3)``.  The odd Human-Note coefficient is first converted
  from the real BRY convention by ``C_HN^(1)=i*C_BRY_tilde``; consequently
  its two-pants product supplies a second minus sign rather than silently
  identifying two inequivalent coefficient conventions;
...
PYTHON_DIR = SCRIPT_DIR.parent / "genus_2_cross_channel"
for dependency_directory in (CODE_ROOT, PYTHON_DIR):
...
from genus_2.glasses_partition import (  # noqa: E402
    glasses_diagonal_sector_contribution,
    glasses_null_transport,
    glasses_sector_pair,
)
from genus_2.theta_partition import (  # noqa: E402
...
class ChannelResult:
    ...
    block_calls: int
    sector_contributions: tuple[float, float]
    partition_signs: tuple[int, int]
...
class NSGenus2CRecursion:
    ...
    def _regular(
        ...
        if self.channel == "glasses":
            # Direct PBW factorization through total level four fixes the
            # glasses vacuum/global convolution to the ordinary product.  The
            # two self-loop incidences at each trinion supply the same Koszul
            # factor that would otherwise arise from polarizing
            # Q_gl=e_bridge(e_left+e_right), so inserting a second polarized
            # sign here double-counts it.  In particular the old odd-sector
            # flips of both vacuum handle lifts fail already at twice-level
            # (0,3,1).
            return self._vacuum(lifts) * self._global(weights, sector, lifts)
        result = 0.0 + 0.0j
        for sigma in (
            (e0, e1, e2)
            for e0 in (0, 1)
            ...
            for edge in range(3):
                for r in range(2, remaining + 1):
                    for s in range(1, remaining // r + 1):
                        ...
                        shifted[edge] += rs / 2.0
                        if self.channel == "theta":
                            transport = theta_null_transport(
                                sector=current_sector,
                                lifts=current_lifts,
                                edge=edge,
                                rs=rs,
                            ...
                            if transport.child_sector != child_sector:
                                raise AssertionError(
                                    "theta residue and parity transport disagree"
                                )
                            child_lifts = transport.child_lifts
                            edge_character = transport.edge_character
                            ...
                            if transport.child_sector != child_sector:
                                raise AssertionError(
                                    "glasses residue and parity transport disagree"
                                )
                            child_lifts = transport.child_lifts
                            edge_character = transport.edge_character
                            orientation_constant = 1
...
def _spin_characteristic_from_lifts(
    ...
    zero.  In the theta marking there is an affine second-bit shift.  Direct
    physical-Majorana pants sewing with the Human Note orientation gives

    ``beta = (eta_zero*eta_infinity < 0, eta_one*eta_infinity > 0)``.

    Equivalently, after :func:`_theta_schottky_data` returns generator signs
    ``(s1,s2)``, the map is ``beta=(s1>0,s2<0)``.  The second inequality is
    essential: treating both bits linearly selects the wrong even spin
    structure.  Therefore human-note theta lifts ``(+,-,+)`` (or their
    simultaneous negative) realize ``[00|00]`` and match glasses lifts
    ``(+,+,+)``.  This map concerns the *physical* Majorana determinant in
    ``Q_L``; it is unrelated to the auxiliary Majorana star quotient used by
    the double-Virasoro block algorithm.
    """
...
def _free_superfield_partition_cached(
    *,
    ...
    # This is the physical free scalar + physical NS Majorana denominator of
    # Q_L.  It is not the auxiliary Majorana introduced to branch
    # SCA x F_aux into Vir x Vir.  Bosonization for a complex physical fermion
    # gives Z_psi^2 = theta_delta Z_M.  One real physical Majorana therefore
    # contributes sqrt(theta_delta P_bos) in the chiral theory.  Multiplying
    # by the real scalar and both chiralities gives
    #
    ...
    return FreeSuperfieldDiagnostics(
        ...
        fermion_method=(
            "physical NS Majorana: exact theta-function bosonization; "
            "no auxiliary double-Virasoro fermion"
        ),
    )
...
def _structure_weight(
    ...
    r"""Return the two-pants coefficient in the Human-Note convention.

    ``ns_tilde_structure_constant`` is real for positive momenta because it
    is the BRY coefficient of the positive-norm local top component
    ``W=G_{-1/2}\widetilde G_{-1/2}V``.  The Human Note instead factorizes
    that top component into two odd chiral states.  The graded tensor-product
    norm gives the exact boundary dictionary

    ``C_HN^(0)=C_BRY`` and ``C_HN^(1)=i*C_BRY_tilde``.

    Either sign of ``i`` gives the same genus-two result because every term
    contains two pants coefficients.  Keeping the phase here is essential:
    the explicit ``(-1)^a`` in the Human-Note nonchiral sewing formula is a
    separate descendant/Koszul sign and must remain in the assembler.
    """

    if sector not in (0, 1):
        raise ValueError("sector must be zero or one")

    def coefficient(*arguments: float) -> complex:
        if sector == 0:
            return ns_structure_constant(*arguments, precision=precision)
        return 1j * ns_tilde_structure_constant(
            *arguments, precision=precision
        )

    if channel == "theta":
        value = coefficient(*momenta) ** 2
    elif channel == "glasses":
        left, right, bridge = momenta
        value = coefficient(left, bridge, left) * coefficient(
            right, bridge, right
        )
    else:
        raise ValueError("channel must be theta or glasses")
...
def evaluate_channel(
    *,
    ...
    free_max_mode: int,
    free_superfield_override: float | None = None,
) -> tuple[ChannelResult, FreeSuperfieldDiagnostics | None]:
    started = time.time()
    ...
    total = 0.0
    integrated_sector_totals = [0.0, 0.0]
        ...
        for i1, p1 in enumerate(rules[1][0]):
            for i2, p2 in enumerate(rules[2][0]):
                ...
                for sector in (0, 1):
                    ...
                    if channel == "theta":
                        sector_contribution = theta_diagonal_sector_contribution(
                            sector=sector,
                            measure=float(measure),
                            structure_weight=structure_cache[structure_key],
                            primary_times_block=primary * block,
                            primary_parities=TYPE0B_NS_PRIMARY_PARITIES,
                        )
                    else:
                        sector_contribution = glasses_diagonal_sector_contribution(
                            sector=sector,
                            measure=float(measure),
                            structure_weight=structure_cache[structure_key],
                            primary_times_block=primary * block,
                    ...
                    integrated_sector_totals[sector] += float(
                        sector_contribution
                    )
                    total += float(sector_contribution)
    if free_superfield_override is None:
        ...
        free_value = float(free.value)
    else:
        free = None
        free_value = float(free_superfield_override)
        if not math.isfinite(free_value) or free_value <= 0.0:
            raise ValueError("free_superfield_override must be finite and positive")
    q_l = total / free_value**9
    return (
        ChannelResult(
            channel=channel,
            ...
            sector_contributions=tuple(integrated_sector_totals),
            partition_signs=(
                tuple(
                    theta_sector_pair(
                        sector,
...
def run_internal_checks() -> dict[str, object]:
    ...
    # independent physical-Majorana pants-sewing oracle fixes which theta
    # plumbing lifts realize that transported characteristic in current
    # (zero,one,infinity) edge order: (+,-,+), not (+,+,+).
    ...
    if alternative_theta_characteristic != ((0, 0), (0, 1)):
...
def _plot(path: Path, rows: Sequence[dict[str, object]]) -> None:
    ...
    plotted_values = [float(row["q_l"]) for row in rows]
    if plotted_values and all(value > 0 for value in plotted_values):
        axis.set_yscale("log")
    elif plotted_values:
        nonzero = [abs(value) for value in plotted_values if value != 0]
        linear_threshold = min(nonzero) / 10.0 if nonzero else 1.0
...
def run(argv: Sequence[str] | None = None) -> dict[str, object]:
    ...
    # This historical overlap CSV predates the branch-composed period marking
    # used by the current production config.  Keep its original lifts here and
    # fail closed below when the physical spin characteristics differ.  Use
    # genus_2/rerun_human_note_genus2.py for a current same-spin computation.
    ...
    if len(set(spin_characteristics.values())) != 1:
        raise RuntimeError(
            "theta and glasses plumbing lifts select different physical spin "
            "structures in this legacy period marking; use the branch-composed "
            "current-config runner instead"
        )
    settings = []
    ...
    output = {
        "scope": (
            "Genus-two NS super-Liouville theta/glasses c-recursion; "
            "both channels use parity-correct nonchiral sewing"
        ),
        ...
        "free_denominator_scope": {
            ...
            "formula": (
                "det(Im Omega)^(-1/2) |theta[delta](0|Omega)| |P_X|^3"
            ),
            "nonchiral_sign": (
        ...
        "nonchiral_assembly_scope": {
            "theta": "parity-correct formula from Human Notes/SCblock.tex",
