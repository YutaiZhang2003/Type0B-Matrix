...
def ns_c_pole_mp(r: int, s: int, weight) -> NSPoleMP:
    """Return the physical-sheet ``c_(r,s)(h)`` and ``J=-dc/dh``.
    ...
    For positive real ``h`` the standard pole is the root with the larger
    numerator ``4h+rs-1 +/- sqrt(D)``.  Continuing a complex weight across a
    principal-square-root cut can otherwise exchange that root with the
    companion one and, for ``s=1``, spuriously set ``b^2`` exactly to zero.
    Selecting the larger numerator is the algebraically stable continuation
    of the positive-real sheet.  The remaining ``b`` square root is principal;
    ordinary-edge residue products are invariant under its common sign.
    """
    ...
    affine = 4 * h + rs - 1
    if abs(affine - discriminant) > abs(affine + discriminant):
        discriminant = -discriminant
    x = -(affine + discriminant) / (r * r - 1)
    b = mpmath.sqrt(x)
