...
def plumbing_geometry(
    ...
    *,
    ...
    """Check pairwise disjointness of the standard symmetric plumbing disks."""
    ...
    if topology == "theta":
        ...
        # On each sphere the handle uses the 0/infinity punctures and the
        # separating bridge uses the puncture at 1.
        margins = (
            _sphere_disk_margins(radii[0], radii[2], radii[0]),
            _sphere_disk_margins(radii[1], radii[2], radii[1]),
        )
    else:
        raise ValueError(f"unknown topology {topology!r}")
    minimum = min(value for sphere in margins for value in sphere)
    ...
    *,
    ...
    if topology == "theta":
        ...
        if frozen_bridge:
            ...
            upper = np.asarray([np.inf, 110.0, np.inf, 110.0], dtype=float)
            ...
            upper = np.asarray([np.inf, 110.0, np.inf, 110.0, bound, bound], dtype=float)
            ...
            def unpack(values: np.ndarray):
                ...
                q = (q_from_tau(taus[0]), q_from_tau(taus[1]), bridge_value)
