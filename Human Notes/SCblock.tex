...
\ie \label{NSblock}
\mathbf{F}_a(h_1, h_2, h_3, c)= & \sum_{|A|=|B|,|C|=|D|,|E|=|F|, (-1)^{A+C+E}=(-1)^a} q_1^{|A|}\eta_1^A q_2^{|C|} \eta_2^C q_3^{|E|} \eta_3^E \mathbf{G}_{h_1}^{A B} \mathbf{G}_{h_2}^{C D} \mathbf{G}_{h_3}^{E F} \\
...
\fe
At a pole $c_{rs}$ on the complex $c$-plane, the residues are contributed by the descendents of this null state, which we will call $\chi_{rs}=\sum_{M} \chi_{rs}^M \mathbf{L}_{-M}\phi$. The pole of $\mathbf{F}_a$ at $c=c_{rs}(h_1)$ is given by
\ie
...
\ie
\rho_a(\mathbf{L}_{-A}\chi_{rs} , \mathbf{L}_{-B}\phi_2,  \mathbf{L}_{-C}\phi_3) = (-1)^{rsE} \begin{cases} \rho_{a}(\mathbf{L}_{-A}\phi_{h_{rs}+\frac{rs}{2}} , \mathbf{L}_{-B}\phi_2,  \mathbf{L}_{-C}\phi_3) \mathbf{P}_{rs}^a(h_2, h_3),\quad rs\in 2\mathbb{Z}, \\ 
\rho_{1-a}(\mathbf{L}_{-A}\phi_{h_{rs}+\frac{rs}{2}} , \mathbf{L}_{-B}\phi_2,  \mathbf{L}_{-C}\phi_3) \mathbf{P}_{rs}^{a}(h_2, h_3),\quad rs\in 2\mathbb{Z}+1,
\end{cases}
\fe
...
\ie
\mathbf{W}^{(1)}_{a,rs} &= -\frac{\partial c_{rs}(h_1)}{\partial h_1} \mathbf{A}_{rs}^c(h) q_1^{rs/2}\eta_1^{rs} [\mathbf{P}_{rs}^a(h_2, h_3)]^2\\
...
\ie
\lim_{c\rightarrow \infty}\mathbf{F}_a(h_1, h_2, h_3,c) &= \sum_{\substack{a_1,a_2,a_3\in\{0,1\}\\a_1+a_2+a_3\equiv a\ (\mathrm{mod}\ 2)}} \mathbf{F}_0(0,0,0,\infty;\eta_i\rightarrow (-1)^{a_i}\eta_i) \times \mathbf{F}_{a_1a_2a_3}^{\mathfrak{osp}(1|2)}(h_1,h_2,h_3),
\fe
where the $\mathfrak{osp}(1|2)$ block is defined for $a_i\in\{0,1\}$ and $a_1+a_2+a_3\equiv a\ (\mathrm{mod}\ 2)$ by
\ie
\mathbf{F}^{\mathfrak{osp}(1|2)}_{a_1a_2a_3}(h_1, h_2, h_3) &= \sum_{k_1, k_2, k_3} q_1^{k_1+\frac{1}{2}a_1}\eta_1^{a_1}q_2^{k_2+\frac{1}{2}a_2}\eta_2^{a_2}q_3^{k_3+\frac{1}{2}a_3}\eta_3^{a_3} (-1)^{a_1a_2+a_1a_3+a_2a_3}\\
...
and hence
\ie
\mathbf{F}^{\mathfrak{osp}(1|2)}_{a_1a_2a_3}(h_1, h_2, h_3) &= \sum_{k_1, k_2, k_3} q_1^{k_1+\frac{1}{2}a_1}\eta_1^{a_1}q_2^{k_2+\frac{1}{2}a_2}\eta_2^{a_2}q_3^{k_3+\frac{1}{2}a_3}\eta_3^{a_3}(-1)^{a_1a_2+a_1a_3+a_2a_3} \left[\prod_{i=1}^3 \frac{1}{k_i!(2h_i)_{k_i+a_i}}\right]\\
...
It can be acting acting arbitrary $\psi_{-r}, L_{-n}, G_{-r}$ on $\mathbf{1}_{\mathsf{F}}\otimes \phi$. 
...
\fe
where ${\Psi}_{-\mathsf{A}}v$ stands for $\Psi_{-\mathsf{A}}v \equiv \psi_{-r_1}\cdots \psi_{-r_m} v$, $|\mathsf{A}|$ stands for the total level of fermion lowering operators, and $(-1)^{\mathsf{A}}$ is the total $\psi$ parity. We have used the fact that $\langle \mathbf{L}_{-A}v| \Psi_{-\mathsf{A}}^\dagger \Psi_{-\mathsf{B}}|\mathbf{L}_{-A}v\rangle$ is the matrix $(-1)^{\mathsf{A}}\delta_{\mathsf{A},\mathsf{B}}\mathbf{G}_{AB}$, which give rises to the sign $(-1)^{\mathsf{A}+\mathsf{B}+\mathsf{C}}$. The 3-point function $\hat{\rho}^{L/R}$ is labeled by the total parity, including the fermionic one.

For states $u\otimes x$, the 3-point function can be factorized into
\ie
& \hat{\rho}_a\left(u_1 \otimes x_1, u_2 \otimes x_2, u_3 \otimes x_3\right)=(-1)^{\left|x_1\right|\left(\left|u_2\right|+\left|u_3\right|\right)+\left|x_2\right|\left|u_3\right|} \rho^{\mathsf{F}}(u_1, u_2, u_3) \rho_a(x_1, x_2, x_3),
\fe
where $\rho^{\mathsf{F}}(u_1, u_2, u_3)$ is the 3-point function in the free fermion theory. The sign $(-1)^{\left|x_1\right|\left(\left|u_2\right|+\left|u_3\right|\right)+\left|x_2\right|\left|u_3\right|}$ comes from moving operator order, and the additional sign $(-1)^{a|u_3|}$ comes from Importantly, $\rho^{\mathsf{F}}$ is only nonzero if $\mathsf{A}+\mathsf{B}+\mathsf{C}\equiv 0$ mod 2. If we define the free fermion block as
\ie
...
\fe
where we have used the fact that $h_{n}^{(1)}+h_{n}^{(2)}-h = 2n^2$ and that $v_n$ has Grassmann parity $2n$. $G_h^{AB}$ is the Virasoro inverse Gram matrix.
...
\ie
\mathbf{B}_a(h_1, h_2, h_3;n_1, n_2, n_3) = \frac{\hat{\rho}_a(v_{1,n_1}, v_{2,n_2}, v_{3,n_3})}{\|v_{1,n_1}\|\|v_{2,n_2}\|\|v_{3,n_3}\|},
\fe
...
&\qquad \times [\mathbf{B}_a(h_1,h_2,h_3;n_1,n_2,n_3)]^2 F(h_{1,n_1}^{(1)},h_{2,n_2}^{(1)},h_{3,n_3}^{(1)},c^{(1)})F(h_{1,n_1}^{(2)},h_{2,n_2}^{(2)},h_{3,n_3}^{(2)},c^{(2)}),
\fe
...
The remaining problem is to compute the branching coefficient $\mathbf{B}_a(h_1,h_2,h_3;n_1,n_2,n_3)$. This has been worked out in \cite{1312.4520, 1406.3008} (they called it the blow up factor). To compute this factor, let us define the normalization of $v_n$ by the following. We first introduce a free field representation of the $\mathsf{SCA}_{1/2}$ algebra. Introducing free boson operators $c_n$ and free fermion operators $\eta_r$ (called $\psi_r$ in \cite{1608.02568}). The $\mathsf{SCA}_{1/2}$ generators are given by
\ie \label{freefieldrep}
\begin{aligned}
...
\end{aligned}
\fe
and one can define a representation $\mathcal{M}(P)$ of $\mathsf{SCA}_{1/2}\otimes \mathsf{F}_{1/2}$ algebra by acting $\eta_{-r}, c_{-n}, \psi_{-r}$ on a state $\phi_P$ labelled by $P$ that satisfies $\hat{P}\phi(P) = P\phi(P)$ and annihilated by all positive modes. For $P\notin \frac{1}{2}(b\mathbb{Z}+b^{-1}\mathbb{Z})$, this representation is irreducible and is isomorphic to the highest weight representation $\mathcal{M}_{\NS}(h)$ with $h = \frac{1}{8}Q^2-\frac{1}{2}P^2$. 

The $\mathsf{Vir}\oplus\mathsf{Vir}$ primaries for $n>0$ in $\mathcal{M}(P)$ can be directly constructed as
\ie
...
\ie
\hat{\rho}_a&(v_{1,n_1},v_{2,n_2},v_{3,n_3}) = (-1)^{n_1+n_2+n_3}i^a 2^{-\left(n_1+n_2+n_3\right)}  \\
...
\ie
\mathbf{B}_a&(h_1, h_2, h_3;n_1, n_2, n_3) = \frac{(-i)^a}{\prod_{i=1}^3\sqrt{\ell(2P_i, 4n_i)\ell(Q+2P_i, 4n_i)}}\\
&\quad \times \ell\left(\tfrac{Q}{2}+P_1+P_2+P_3, 2(n_1+n_2+n_3)\right)\ell\left(\tfrac{Q}{2}-P_1+P_2+P_3, 2(-n_1+n_2+n_3)\right)\\
&\quad \times \ell\left(\tfrac{Q}{2}+P_1+P_2+P_3, 2(n_1+n_2+n_3)\right) \ell\left(\tfrac{Q}{2}+P_1+P_2+P_3, 2(n_1+n_2+n_3)\right).
\fe
...
&=(-1)^{A\mathsf{A}+(B+|\alpha|)(\mathsf{C+c})}\rho(\Psi_{-\mathsf{A}}\mathbf{1}_{\mathsf{F}}, \boldsymbol\Psi_{-\mathsf{B}}u^{\mathsf{b}},\boldsymbol\Psi_{-\mathsf{C}}u^{\mathsf{c}}) {\rho}_f^{(\eta)}(\mathbf{L}_{-A}\phi_1, \mathbb{L}_{-B}w^\alpha_2, \mathbb{L}_{-C}w^\gamma_3) 
\fe
...
\fe
where the fermionic 3-point function is normalized such that $\rho(\mathbf{1}_{\mathsf{F}}, u^0, u^0)=1,\rho(\mathbf{1}_{\mathsf{F}}, u^1, u^1)=i$. One then have the decomposition
\ie \label{Fconvolution}
...
\widehat{\mathbb{F}}^{(\eta, \eta')}_f&(h_1, \beta_2, \beta_3, c) = \sum_{\substack{
  |A|=|A'|, |B|=|B'|, |C|=|C'|\\
  |D|=|D'|, |E|=|E'|, |F|=|F'|\\
  n_1\in \frac12\mathbb{Z},\ n_2, n_3\in \frac{1}{2}\mathbb{Z}+\frac{1}{4}\\
...
\fe
and therefore,
\ie \label{rextendedblock}
\widehat{\mathbb{F}}_f^{(\eta,\eta')}(h_1, \beta_2, \beta_3, c) &= \sum_{\substack{n_1, n_2, n_3\in \frac{1}{2}\mathbb{Z}+\frac{1}{4}\\(-1)^{2n_1+\alpha_2+\alpha_3}=(-1)^f}} q_1^{2n_1^2}q_2^{2n_2^2-\frac{1}{8}}q_3^{2n_3^2-\frac{1}{8}}\eta_1^{2n_1}\eta_2^{\alpha_2}\eta_3^{\alpha_3}\\
&\quad \times (-1)^{2n_1\alpha_2+2n_1\alpha_3+\alpha_2\alpha_3} F(h_{1,n_1}^{(1)},h_{2,n_2}^{(1)},h_{3,n_3}^{(1)},c^{(1)})F(h_{1,n_1}^{(2)},h_{2,n_2}^{(2)},h_{3,n_3}^{(2)},c^{(2)})\\
&\quad \times \mathbb{B}^{(\eta)}_f(h_1, \beta_2, \beta_3,c;n_1, n_2, n_3,\alpha_2, \alpha_3) \mathbb{B}^{(\eta')}_f(h_1, \beta_2, \beta_3,c;n_1, n_2, n_3,\alpha_2, \alpha_3),
\fe
where the R-sector branching coefficients are defined as
\ie
\mathbb{B}^{(\eta)}_f(h_1, \beta_2, \beta_3,c;n_1, n_2, n_3,\alpha_2, \alpha_3) = \frac{\hat{\rho}_f^{(\eta)}(v_{1,n_1}, v_{2,n_2}^{\alpha_2}, v_{3,n_3}^{\alpha_3})}{\|v_{1,n_1}\|\|v_{2,n_2}^{\alpha_2}\|\|v_{3,n_3}^{\alpha_3}\|}.
\fe
We will suppress the labels $h_1, \beta_2, \beta_3, c$ in $\mathbb{B}^{(\eta)}_f$ from now on.

\subsubsection*{Ramond branching coefficients}

Unlike the NS branching coefficient $\mathbb{B}_a$, the Ramond branching coefficient $\mathbb{B}_f^{(\eta)}$ is not computed in the literature. There are conjectures in \cite{1312.4520} about its structure, but the numerical prefactor remains unknown. In the rest of this section, we develop a recursion relation between $\mathbb{B}^{(\eta)}_f(n_1, n_2, n_3, \alpha_2, \alpha_3)$'s, which can be used to solve them numerically.
...
&\qquad = \left[\sum_{i=1}^2 (\mathbb{V}_{n_2,\alpha_2}^{(i)}-\mathbb{V}_{n_3,\alpha_3}^{(i)})(h_{1,n_1}^{(i)}-h_{2,n_2}^{(i)}-h_{3,n_3}^{(i)})\right]\hat{\rho}_f^{(\eta)}(v_{1,n_1}, v_{2, n_2}^{\alpha_2}, v_{3, n_3}^{\alpha_3})\\
&\qquad + \sum_{|C|+|D|=4n_2-1} \mathbb{V}_{-1,n_2, \alpha_2}^{CD}\rho(\nu_{h_{1,n_1}^{(1)}},L_{-C}\nu_{h_{2,n_2-1}^{(1)}}, \nu_{h_{3,n_3}^{(1)}})\rho(\nu_{h_{1,n_1}^{(2)}},L_{-D}\nu_{h_{2,n_2-1}^{(2)}}, \nu_{h_{3,n_3}^{(2)}})\hat{\rho}_f^{(\eta)}(v_{1,n_1},v_{2,n_2-1}^{\alpha_2},v_{3,n_3}^{\alpha_3})\\
&\qquad + \sum_{|E|+|F|=4n_3-1} \mathbb{V}_{-1,n_3, \alpha_3}^{EF}\rho(\nu_{h_{1,n_1}^{(1)}},\nu_{h_{2,n_2}^{(1)}}, L_{-E}\nu_{h_{3,n_3-1}^{(1)}})\rho(\nu_{h_{1,n_1}^{(2)}},\nu_{h_{2,n_2}^{(2)}}, L_{-F}\nu_{h_{3,n_3-1}^{(2)}})\hat{\rho}_f^{(\eta)}(v_{1,n_1},v_{2,n_2}^{\alpha_2},v_{3,n_3-1}^{\alpha_3}),
\fe
where we have used the fact that $\rho(\nu_1,L_{-1}\nu_2,\nu_3)=h_1-h_2-h_3,\rho(\nu_1,\nu_2,L_{-1}\nu_3)=-h_1+h_2+h_3$ and the factorization of 3-point functions. Therefore,
{\footnotesize
\ie
&\left[\sum_{i=1}^2 (\mathbb{V}_{n_2,\alpha_2}^{(i)}-\mathbb{V}_{n_3,\alpha_3}^{(i)})(h_{2,n_2}^{(i)}+h_{3,n_3}^{(i)}-h_{1,n_1}^{(i)})\right]\mathbb{B}_f^{(\eta)}(n_1,n_2,n_3,\alpha_2,\alpha_3)=\\
& - \sum_{|A|+|B|=4n_1-3}\mathbf{V}_{1,n_1}^{AB}\rho(L_{-A}\nu_{h_{1,n_1-1}^{(1)}}, \nu_{h_{2,n_2}^{(1)}}, \nu_{h_{3,n_3}^{(1)}})\rho(L_{-B}\nu_{h_{1,n_1-1}^{(2)}}, \nu_{h_{2,n_2}^{(2)}}, \nu_{h_{3,n_3}^{(2)}})\mathbb{B}_f^{(\eta)}(n_1-1,n_2,n_3,\alpha_2,\alpha_3)\frac{\|v_{n_1-1}\|}{\|v_{n_1}\|}\\
& + \sum_{|C|+|D|=4n_2-1} \mathbb{V}_{-1,n_2, \alpha_2}^{CD}\rho(\nu_{h_{1,n_1}^{(1)}},L_{-C}\nu_{h_{2,n_2-1}^{(1)}}, \nu_{h_{3,n_3}^{(1)}})\rho(\nu_{h_{1,n_1}^{(2)}},L_{-D}\nu_{h_{2,n_2-1}^{(2)}}, \nu_{h_{3,n_3}^{(2)}})\mathbb{B}_f^{(\eta)}(n_1,n_2-1,n_3,\alpha_2,\alpha_3)\frac{\|v_{n_2-1}^{\alpha_2}\|}{\|v_{n_2}^{\alpha_2}\|}\\
& + \sum_{|E|+|F|=4n_3-1} \mathbb{V}_{-1,n_3, \alpha_3}^{EF}\rho(\nu_{h_{1,n_1}^{(1)}},\nu_{h_{2,n_2}^{(1)}}, L_{-E}\nu_{h_{3,n_3-1}^{(1)}})\rho(\nu_{h_{1,n_1}^{(2)}},\nu_{h_{2,n_2}^{(2)}}, L_{-F}\nu_{h_{3,n_3-1}^{(2)}})\mathbb{B}_f^{(\eta)}(n_1,n_2,n_3-1,\alpha_2,\alpha_3)\frac{\|v_{n_3-1}^{\alpha_3}\|}{\|v_{n_3}^{\alpha_3}\|},
\fe
}
...
\fe
Using this recursion relation, one can solve for the Ramond branching coefficient $\mathbb{B}_f^{(\eta)}(n_1,n_2,n_3,\alpha_2,\alpha_3)$ recursively. One should note that this form only works for $n_1\geq 1$ and $n_2,n_3\geq \frac{3}{4}$. At the boundary values, $L_1$ annihilates $v_{0,\pm \frac{1}{2}}$ and $v_{\pm \frac{1}{4}}^{\alpha}$. For $L_{-1}$, one can use 
\ie
...
&\rho_f(\mathbf{L}_{-A}\chi_{rs},
        ...
        \mathbf{L}_{-E}\phi_3)
\\
&\quad=(-1)^{rsE}\mathbf{P}_{rs}^{f}(h_3,h_2;c_{rs})
\begin{cases}
\rho_f(\mathbf{L}_{-A}\phi_{h_{rs}+rs/2},
       \mathbf{L}_{-C}\phi_2,
       \mathbf{L}_{-E}\phi_3),&rs\in2\mathbb Z,\\[0.3em]
\rho_{1-f}(\mathbf{L}_{-A}\phi_{h_{rs}+rs/2},
       \mathbf{L}_{-C}\phi_2,
       \mathbf{L}_{-E}\phi_3),&rs\in2\mathbb Z+1.
\end{cases}
...
&\rho_f(\mathbf{L}_{-A}\phi_1,
        ...
        \mathbf{L}_{-E}\phi_3)\\
&\quad=\mathbf{P}_{rs}^{f}(h_3,h_1;c_{rs})
\begin{cases}
\rho_f(\mathbf{L}_{-A}\phi_1,
       \mathbf{L}_{-C}\phi_{h_{rs}+rs/2},
       \mathbf{L}_{-E}\phi_3),&rs\in2\mathbb Z,\\[0.3em]
\rho_{1-f}(\mathbf{L}_{-A}\phi_1,
       \mathbf{L}_{-C}\phi_{h_{rs}+rs/2},
       \mathbf{L}_{-E}\phi_3),&rs\in2\mathbb Z+1,
\end{cases}
...
&\quad=(-1)^{rs(A+C)}\mathbf{P}_{rs}^{f}(h_1,h_2;c_{rs})
\begin{cases}
\rho_f(\mathbf{L}_{-A}\phi_1,
       \mathbf{L}_{-C}\phi_2,
       \mathbf{L}_{-E}\phi_{h_{rs}+rs/2}),&rs\in2\mathbb Z,\\[0.3em]
\rho_{1-f}(\mathbf{L}_{-A}\phi_1,
       \mathbf{L}_{-C}\phi_2,
...
\rho_f(\mathbf{L}_{-A}\chi_{rs},
        \mathbf{L}_{-C}\phi_2,
        \mathbf{L}_{-E}\chi_{rs})&=(-1)^{rs(f+rs)}
