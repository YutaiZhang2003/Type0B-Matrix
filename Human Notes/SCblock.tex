...
\subsection{h-recursion}

We now review the derivation of h recursion relation for the  tours n point block in the necklace channel.
Consider the tours with complex structrue $\tau$, and the flat coordinate on the torus has $z\sim z+2\pi\sim z+2\pi \tau$. We consider $N$ primary operators inserted at positions $z=w_i$ for $i=1,\cdots,N$. We set $w_N=0$ by convention, and write $w_i=2\pi(\tau-\sum_{k=1}^i\tau_k)$. In the necklace channel, the torus is decomposed into $N$ cylinders, of moduli $\tau_1, \tau_2, \cdots,\tau_N$, with $\sum_{k=1}^N\tau_k=\tau$. We will also write $q_i=e^{2\pi i\tau_i}$.  The torus $N$-point block is defined as
\ie\label{necklaceblock}
&F\left(q_1,h_1,d_1,...,q_N,h_N,d_N,c\right)
\\
=&\sum_{n_1,...,n_N=0}^{\infty}\left(\prod_{i=1}^{N}q_i^{n_i}\right)\sum_{|A_1|=|B_1|=n_1}...\sum_{|A_N|=|B_N|=n_N}\left[\prod_{i=1}^N\left(G_{h_i}\right)^{A_iB_i}\rho(L_{-B_i}\nu_i,\mu_i,L_{-A_{i+1}}\nu_{i+1})\right].
\fe
where $h_i$ denotes the weight of the intermediate primaries $\nu_i$ and $d_i$ denote the conformal weight of external primaries $\mu_i$.

The idea of the $h$ recursion relation is to analyze the analytic structure of the conformal block as a meromorphic function in $h_i$, sharing the same spirit as the c recursion. The gram matrix would be degenerate at $h_i=h_{r_i,s_i}(c)$, where a singular vector $\chi_{r,s}$ with weight $h_{r,s}+rs$ is developed. For a generic $c$, the matrix element of the inverse gram matrix will develop a simple pole. Therefore, one can make the following ansatz on the Virasoro block
\ie
F(q_i,h_i,d_i;c)= F_{\text{reg}}(q_i,h_i,d_i;c)+ \sum_{i=1}^N \sum_{r_i\geq 1, s_i\geq 1}\frac{B^{(i)}_{r,s}}{h_i- h_{r,s}(c)}.
\fe
where $F^{\text{reg}}$ has no pole on the entire $h_i$ plane. The rest of the work will be devoted to pinning down the unknown factor $B^{(i)}_{r,s}$ and $F^{\text{reg}}(q_i,h_i,d_i;c)$.

The factor $B^{(i)}_{r,s}$ can be extracted by considering the limit 
\ie
B^{(i)}_{r,s}=\lim_{h_i\rightarrow h_{r,s}} \left(h_i-h_{r,s}(c)\right) F(q_i,h_i,d_i;c).
\fe
Under this limit, only the singular vector $\chi_{r,s}$ inside the Verma module of $\nu_i$,  as well as the descendant of $\chi_{r,s}$ will contribute to the Virasoro block. Following from the same derivation in the c-recursion, we have 
\ie\label{virasoro_Res}
B^{(i)}_{r,s}(h_1,\dots \hat{h}_i, h_N) =A_{r,s}^c q_i^{rs} P_c^{rs}(h_{i-1}, d_{i-1}) P_c^{rs}(h_{i+1}, d_{i}) F(h_i\rightarrow h_{r,s}+rs, c),
\fe 
Now let us try to extract the regular part $F_{\text{reg}}$. The key step is to change the variable from $\{h_i |i=1,\dots N \}$ to $\{h_1,a_i=h_i-h_1 |i=2,\dots N \}$, with $a_1=0$ as the bookkeeping notation. Then we have
\ie
F(q_i,h_i,d_i;c)= F_{\text{reg}} (h_1,a_2\dots ,a_N)+ \sum_{i=1}^N \sum_{r_i\geq 1, s_i\geq 1}\frac{B^{(i)}_{r,s}}{h_1+a_i- h_{r,s}(c)}.
\fe
In this new variable, $B^{(i)}_{r,s}(h_1,\dots \hat{h}_i, h_N)$ can be viewed as the limit
\ie
B^{(i)}_{r,s}=\lim_{h_1\rightarrow -a_i+h_{r,s}} \left(h_1+a_i-h_{r,s}(c)\right) F.
\fe
It can thus be treated as a function of $a_2\dots a_N$ and independent of $h_1$. It allows us to extract $F^{\text{reg}}$ by analyzing the large $h_1$ behavior of Virasoro block with $a_i$ keep fixed, where the singular term all get suppressed. As we will show, the strict large $h_1$ limit for the Virasoro block exist, and therefore $F^{\text{reg}} (h_1,a_2\dots ,a_N)$ has no pole at $h_1\rightarrow \infty$. By the Liouville theorem of analytic function, the regular part must be independent of $h_1$. Therefore, the regular part can be fully determined by the limit
\ie
F_{\text{reg}} (q_i, a_i,d_i ; c) =\lim_{h_1\rightarrow \infty} F(q_i,h_i,d_i;c).
\fe
Now we will present the analysis for the large $h$ behavior of the Virasoro block, and compute the regular part $F_{\text{reg}}$ explicitly. 

To do the power counting for $h_1$ in the conformal block, it is convenient to introduce an orthogonal basis defined as follows. Let $A=(a_1,\ldots,a_n)$ be a partition, with
$a_1\geq\cdots\geq a_n\geq1$. We write
\ie
[A]=n,
\qquad
|A|=\sum_{i=1}^{n}a_i,
\fe
for the number of rows and the size of the partition, respectively. Introduce a total order on the set of partitions by declaring $A<B$
if either
\begin{enumerate}
    \item $[A]<[B]$; or
    \item $[A]=[B]$ and $a_j<b_j$ at the first index $j$ for which
    $a_j\neq b_j$.
\end{enumerate}

For generic $c$ and $h$, an orthogonal basis of the Verma module
can be constructed by applying the Gram--Schmidt procedure to the
states $L_{-A}|h\rangle$ at each fixed level, in increasing order
with respect to the ordering above. The resulting states take the form
\ie\label{orthob}
\ell_{-A}|h\rangle
=
L_{-A}|h\rangle
+
\sum_{\substack{|B|=|A|\\ B<A}}
k^A_B(c,h)\,L_{-B}|h\rangle.
\fe
The scaling of $k^A_B(c,h)$ can be determined by the orthogonal condition $\langle h| \ell_{-B}^\dagger \ell_{-A} |h\rangle =0$ as well as the scaling of the matrix element 
\ie\label{gram-scaling}
\langle h|L_{-A}^{\dagger}L_{-B}|h\rangle
=
\begin{cases}
C_A h^{[A]}\left(1+\mathcal{O}(h^{-1})\right),
& A=B,\\[4pt]
\mathcal{O}\!\left(h^{[A]-1}\right),
& [A]=[B],\quad A\neq B,\\[4pt]
\mathcal{O}\!\left(h^{\min([A],[B])}\right),
& [A]\neq[B].
\end{cases}
\fe
One has
\ie\label{kscaling}
&k^A_B(c,h) \sim {\cal O}(h^{-1}),~~~ [B]=[A],~B\not=A;
\\
& k^A_B(c,h) \sim {\cal O}(h^0),~~~ [B]<[A].
\fe
Therefore, the norm of the basis state $\ell_{-A}|h\rangle$ satisfies
\ie
\langle h| \ell_{-A}^\dagger \ell_{-A} |h\rangle =\langle h| L_{-A}^\dagger L_{-A} |h\rangle +\dots.
\fe
where $\dots$ denote the subleading term in $h_1$. Under the orthogonal basis, the Virasoro block can be represented as 
\ie\label{hlimit}
F= \sum_{n_1,...,n_N=0}^{\infty}\left(\prod_{i=1}^{N}q_i^{n_i}\right)\sum_{|A_1|=n_1}...\sum_{|A_N|=n_N}\left[\prod_{i=1}^N{\rho(\ell_{-A_i}h_1,d_i,\ell_{-A_{i+1}}h_1)\over\langle h_1|\ell_{-A_i}^\dag\ell_{-A_i}|h_1\rangle}\right].
\fe

We also need to determine the asymptotic of three point function  $\rho(\ell_{-A}\nu_i,\mu_i,\ell_{-B}\nu_{i+1})$. The key observation is that the three point function $\rho(L_{-A}\nu_i,\mu_i,L_{-B}\nu_{i+1})$ satisfy the same conformal ward identity as $G_{AB}=\rho(L_{-A}\nu_i,\mathbf{1},L_{-B}\nu_{i})$  when $h_1$ is large. This is because all the terms in the ward identity that involves the action of Virasoro generators on $\mu_i$ will produce the subleading contribution in $h_1$. Therefore, one can effectively replace $\mu_i$ with the identity operator, with the caveat of replacing $\nu_{i+1}$ in the matrix element with $\nu_i$ to ensure the normalization $\rho(\nu_i , \mathbf{1},\nu_i)=1$ make sense.

Combining this argument with the scaling of $k^A_B$ \eqref{kscaling}, we conclude
\ie
\lim_{h_1\rightarrow \infty} \frac{\rho(\ell_{-A}\nu_i,\mu_i,\ell_{-B}\nu_{i+1})}{\langle h| \ell_{-A}^\dagger \ell_{-A} |h\rangle}=\lim_{h_1\rightarrow \infty} \frac{\rho(L_{-A}\nu_i,\mathbf{1},L_{-B}\nu_{i+1} )+\dots}{\langle h| L_{-A}^\dagger L_{-A} |h\rangle+\dots} = \delta_{AB}.
\fe
Therefore, the large $h_1$ limit of the Virasoro block exist and collapse into 
\ie
\lim_{h_1\rightarrow \infty} F(h_1,a_i,d_i)= \sum_{n=0}^{\infty} p(n) (q_1\dots q_N)^n= \prod_{n=1}^{\infty} \frac{1}{1-(q_1\dots q_N)^n}.
\fe
We can thus identify it as the regular block and conclude the $h$ recursion in the necklace channel as
\ie
F(q_i,h_i,d_i;c)= \prod_{n=1}^{\infty} \frac{1}{1-(q_1\dots q_N)^n} + \sum_{i=1}^N \sum_{r_i\geq 1, s_i\geq 1}\frac{B^{(i)}_{r,s}}{h_i- h_{r,s}(c)}.
\fe
with $B^{(i)}_{r,s}$ defined in \eqref{virasoro_Res}.

\subsection{h-recursion for sphere n point block in elliptic parameter}

In this section, we are going to explore the $h$ recursion for sphere $n$ point block in the elliptic parameter. The idea is to use the fact that the 4-punctured Riemann sphere $S^2$ is conformal equivalent to the pillow geometry $T^2/Z_2$ with flat metric, and write down the recursion relation in the pillow frame. As we will see, it provides an efficient resummation over the puncture position moduli $z_i$ and thus providing a very efficient way of computing sphere n point block.

\subsubsection*{Conformal mapping to pillow geometry}

Use conformal symmetry to fix 4 puncture to be at $(0,z,1,\infty)$.  The conformal map from 4-punctured sphere to pillow geometry can be worked out by identifying the Riemann sphere as the $x$-plane for the elliptic curve\footnote{The genus of the curve can be worked out by treating it as a projective variety inside $\mathbb{CP}^2$ and using the adjunction formula to compute the chern class. Use the fact that the top chern class = eular class, one can compute the eular characteristic of the curve to show that it has $\chi= 0$, namely it has genus 1.}
\ie
y^2= x(x-z)(x-1).
\fe
...
\ie
\omega =\frac{dx}{y} = \frac{dx}{\sqrt{x(x-z)(x-1)}}
\fe
...
\ie
\tau =\frac{\int_{B}\omega}{\int_{A}\omega} =  i\frac{K(1-z)}{K(z)}, 
 \fe
where
\ie
K(z):= \frac{1}{\pi}\int_0^1\frac{d\xi}{\sqrt{\xi(1-\xi)(1-z\xi)}} ={}_2F_1\!\left(\frac12,\frac12;1;z\right).
\fe
The inverse map from $z$ to $\tau$ is given by
\ie
z=\frac{\theta_2(0|\mathfrak{q})^4}{\theta_3(0|\mathfrak{q})^4}, \quad K(z) = \theta_3(0|\mathfrak{q})^2, \quad \mathfrak{q}=e^{\pi i\tau}.
\fe
One can construct the Abel-Jacobi map from the elliptic curve to the flat torus $\mathbb{C}/\Gamma$ via
\ie
w(x)= \frac{1}{\theta_3(\tau)^2}\int_0^{x}  \frac{d\xi}{\sqrt{\xi(\xi-z)(\xi-1)}}
\fe
where the factor $\theta_3(\tau)^2$ is introduced such that $w\cong w+2\pi w\cong w+2\pi \tau$.

\subsubsection*{Sphere 4 point block in pillow frame}
...
\ie
ds^2 =dwd\bar{w} = e^{2\sigma(x,\bar{x})}dx d\bar{x},
\fe
with 
\ie
e^{2\sigma(x,\bar{x})}= \frac{1}{|\theta_3(\tau)|^4} \frac{1}{|x(x-z)(x-1)|}.
\fe
...
={}
\theta_3(0|\tau)^{\frac c2-4\sum_{i=1}^4 h_i}
z^{\frac c{24}-h_1-h_2}
(1-z)^{\frac c{24}-h_2-h_3}
\left\langle\psi_{43}\left|
\mathbb P_h\,\mathfrak{q}^{L_0-\frac c{24}}
\right|\psi_{12}\right\rangle.
\fe

\subsubsection*{$h$ recursion in the pillow frame}

The matrix element can be computed by resolving the projection operator $\mathbb{P}_h$
\ie
\bra{\psi_{34}}\mathbb{P}_h \mathfrak{q}^{L_0-\frac{c}{24}}\ket{\psi_{12}} =\mathfrak{q}^{h-\frac{c}{24}} \sum_{N} \mathfrak{q}^{N} \bra{\psi_{34}} L_{-N}\ket{h}G^{NM} \bra{h}L_{-M}^{\dagger}\ket{\psi_{12}} .
\fe
The subtlety here is that $\bra{\psi_{34}}\ket{h}$ is not normalized. The normalization of this factor can be determined by compare with the defining equation for sphere 4 point block at $z\rightarrow 0$ limit to be 
\ie
\bra{\psi_{43}}\ket{h}\bra{h}\ket{\psi_{12}} =16^{h-\frac{c}{24}}.
\fe
Therefore, we can subtract this factor and introduce
\ie
\mathcal{G}(h_1,h_2,h_3,h_4,h; \mathfrak{q})= (16\mathfrak{q})^{-h+\frac{c}{24}}\bra{\psi_{34}}\mathbb{P}_h \mathfrak{q}^{L_0-\frac{c}{24}}\ket{\psi_{12}} = \sum_{N} \mathfrak{q}^{N} \rho(\psi_{34}, L_{-N}\nu_h) G^{NM} \rho( L_{-M}\nu_h,\psi_{12})
\fe
...
\ie
\mathcal{G}(h_1,h_2,h_3,h_4,h; \mathfrak{q}) = F_{reg}+ \sum_{r,s\geq 1}\frac{C_{r,s}}{h-h_{r,s}(c)}.
\fe
The pole factor $C_{r,s}$ can be extracted by the $h\rightarrow h_{r,s}(c)$ limit, where only the null states $\chi_{r,s}$ will contribute in this sum. The norm factor for the null state follows from \eqref{norm_fusion_poly}. We also have the factoralization relation
\ie
...
\ie
\rho(\psi_{34}, \chi_{r,s}) \rho(\chi_{r,s},\psi_{12})= 16^{rs} P_c^{rs}(h_4, h_3)P_c^{rs}(h_2, h_1).
\fe
Therefore, we have 
\ie\label{residue_hrecursion}
C_{r,s}=A_{r,s}^c (16\mathfrak{q})^{rs} P_c^{rs}(h_{4}, h_{3}) P_c^{rs}(h_{2}, h_{1}) \mathcal{G}(h\rightarrow h_{r,s}+rs, c).
\fe
The regular part of the recursion relation can be determined analyzing the large $h$ asymptotic. One can determine
\ie
F_{reg}= \prod_{n\geq 1}(1-\mathfrak{q}^{2n})^{-1/2}.
\fe
\todo[inline]{The derivation using the conformal ward identity for the pillow states. Require conformal wald identity in pillow geometry}
Therefore, the block $\mathcal{G}$ satisfy the following $h$ recursion relation
\ie
\mathcal{G}(h_1,h_2,h_3,h_4,h; \mathfrak{q}) = \prod_{n\geq 1}(1-\mathfrak{q}^{2n})^{-1/2}+ \sum_{r,s\geq 1}\frac{C_{r,s}}{h-h_{r,s}(c)}.
\fe
with $C_{r,s}$ given in \eqref{residue_hrecursion}. It is convenient to strip off the global block part and define the elliptic block as
\ie
H(h_1,h_2,h_3,h_4,h; \mathfrak{q})= \prod_{n\geq 1}(1-\mathfrak{q}^{2n})^{1/2}\mathcal{G}(h_1,h_2,h_3,h_4,h; \mathfrak{q})
\fe
It would satisfy the following $h$ recursion relation
\ie\label{elliptic_h_4pt}
H(h_1,h_2,h_3,h_4,h; \mathfrak{q}) =1+ \sum_{r,s\geq 1}\frac{B_{r,s}}{h-h_{r,s}(c)}.
\fe
where
\ie
B_{r,s}=A_{r,s}^c (16\mathfrak{q})^{rs} P_c^{rs}(h_{4}, h_{3}) P_c^{rs}(h_{2}, h_{1}) H(h\rightarrow h_{r,s}+rs, c).
\fe
...
z^{\frac c{24}-h_1-h_2}
(1-z)^{\frac c{24}-h_2-h_3} \\
&(16\mathfrak{q})^{h-\frac{c}{24}}  \prod_{n\geq 1}(1-\mathfrak{q}^{2n})^{-1/2} H(h_1,h_2,h_3,h_4,h; \mathfrak{q})
\fe
with $H(h_1,h_2,h_3,h_4,h; \mathfrak{q})$ computed from the recursion relation \eqref{elliptic_h_4pt}. In this formalism, the recursion relation produce a series expansion of sphere 4 point block in $\mathfrak{q}$, which efficiently resum the series in $z$ by
\ie
\mathfrak{q} = \frac{z}{16}+ \frac{z^2}{32} \dots.
\fe
In practical application, $h$ recursion product an efficient algorithm for computing the sphere 4 point block, which converges faster than the c recusion relation.


\subsubsection*{$h$ recursion for sphere n point block in pillow frame}
...
\theta_3(0|\tau)^{\frac c2-4\sum_{i=1}^4 h_i}
z^{\frac c{24}-h_1-h_2}
(1-z)^{\frac c{24}-h_2-h_3} \prod_i \left(\frac{\partial w}{\partial x}(z_i)\right)^{-h_i}
\left\langle\psi_{43}\left|
\mathbb P_h\,\mathfrak{q}^{L_0-\frac c{24}}
\right|\psi_{12}\right\rangle.
\fe
...
\fe
where $n_1 \geq ... \geq n_\ell, r_1>...>r_p$. $\phi$ could be either Grassmann even or Grassmann odd.
...
\subsection{Conformal Block Decomposition in All-NS Plumbing}
...
\subsection{Conformal Block Decomposition in mixed NS/R Plumbing}
...
\ie \label{Rblock}
\mathbb{F}_f^{(\eta, \eta')}(h_1, \beta_2, \beta_3, c) &= \sum_{\substack{|A|=|B|, |C|=|D|, |E|=|F|\\ (-1)^{A+C+E+|\alpha|+|\gamma|}=(-1)^f\\(-1)^{B+D+F+|\alpha'|+|\gamma'|}=(-1)^f}} q_1^{|A|} \eta_1^{A+p_1} q_2^{|C|} \eta_2^{C+|\alpha|} q_3^{|E|} \eta_3^{E+|\gamma|} \mathbf{G}^{AB}_{h_1} \mathbb{G}^{C\alpha, D\alpha'}_{\beta_2} \mathbb{G}^{E\gamma, F\gamma'}_{\beta_3}\\
&\quad\quad \times (-1)^{(A+p_1)(C+|\alpha|)+(A+p_1)(E+|\gamma|)+(C+|\alpha|)(E+|\gamma|)}\\
...
\ie
\operatorname{Res}_{c=c_{rs}^{\R}(\beta_2)} \mathbb{F}_f^{(\eta, \eta')} &= \frac{\partial c_{rs}(\beta_2)}{\partial \beta^2_2}\mathbb{A}_{rs}^c\sum_{\substack{|A|=|B|, |C|=|D|, |E|=|F|\\ (-1)^{A+C+E+|\alpha|+|\gamma|}=(-1)^f\\(-1)^{B+D+F+|\alpha'|+|\gamma'|}=(-1)^f}} q_1^{|A|}\eta_1^{A+p_1} q_2^{|C|}\eta_2^{C+|\alpha|} q_3^{|E|} \eta_3^{E+|\gamma|} \\
&\quad\quad \times (-1)^{(A+p_1)(C+|\alpha|)+(A+p_1)(E+|\gamma|)+(C+|\alpha|)(E+|\gamma|)}\mathbf{G}^{AB}_{h_1} \mathbb{G}^{C\alpha, D\alpha'}_{\beta'_{rs}} \mathbb{G}^{E\gamma, F\gamma'}_{\beta_3}\\
...
&\quad=
\mathbb{P}_{rs}^{\NS,(-1)^{p_1}\eta}(\beta_2,\beta_3;c_{rs})\begin{cases}
\rho_f^{(\eta)}
 (\mathbf{L}_{-A}\phi_{h_{rs}+rs/2},
  \mathbb{L}_{-C}w_{\beta_2}^{\alpha},
  \mathbb{L}_{-E}w_{\beta_3}^{\gamma}),&rs\in2\mathbb Z,\\[0.4em]
(-1)^{p_1+|\gamma|+E}e^{i(-1)^f\pi/4}\rho_{1-f}^{(-\eta)}
 (\mathbf{L}_{-A}\phi_{h_{rs}+rs/2},
  \mathbb{L}_{-C}w_{\beta_2}^{\alpha},
...
\ie
&\operatorname{Res}_{c=c_{r s}\left(h_1\right)} \mathbb{F}_f^{\left(\eta, \eta^{\prime}\right)} = -\frac{\partial c_{r s}\left(h_1\right)}{\partial h_1} \mathbf{A}_{r s}^c(h_1) q_1^{r s / 2} \mathbb{P}_{r s}^{\mathrm{NS}, (-1)^{p_1}\eta}\left(\beta_2, \beta_3 ; c_{r s}(h_1)\right) \mathbb{P}_{r s}^{\mathrm{NS}, (-1)^{p_1}\eta^{\prime}}\left(\beta_2, \beta_3 ; c_{r s}(h_1)\right) \\
...
} q_1^{|A|}\eta_1^{A+p_1} q_2^{|B|} \eta_2^{B+p_2} q_3^{|C|} \eta_3^{C+p_3} \mathbf{G}_{h_1}^{AA'} \mathbf{G}_{h_2}^{BB'} \mathbf{G}_{h_3}^{CC'} \\
& \times (-1)^{(A+\mathsf{A}+p_1)(B+\mathsf{B}+p_2)+(A+\mathsf{A}+p_1)(C+\mathsf{C}+p_3)+(B+\mathsf{B}+p_2)(C+\mathsf{C}+p_3)}(-1)^{\mathsf{A}+\mathsf{B}+\mathsf{C}}q_1^{|\mathsf{A}|}\eta_1^{\mathsf{A}}q_2^{|\mathsf{B}|}\eta_2^{\mathsf{B}}q_3^{|\mathsf{C}|} \eta_3^{\mathsf{C}}\\
...
The three point function could factorize into(this should be viewed as the definition of $\widehat{\rho}_a$, so that the latter double virasoro factorization works)
\ie
&\widehat{\rho}_a\!\left(
\Psi_{-\mathsf A}\mathbf L_{-A}\phi_1,\,
\Psi_{-\mathsf B}\mathbf L_{-B}\phi_2,\,
\Psi_{-\mathsf C}\mathbf L_{-C}\phi_3
\right)
\\
&\qquad =
...
\right).
\fe
where $\rho^{\mathsf{F}}(u_1, u_2, u_3)$ is the normalized 3-point function in the free fermion theory. Importantly, $\rho^{\mathsf{F}}$ is only nonzero if $\mathsf{A}+\mathsf{B}+\mathsf{C}\equiv 0$ mod 2. If we define the free fermion block as
\ie
...
&=\sum_{r_i+s_i=n_i}(-1)^{\sum_{i<j} (r_i(s_j+p_j)+(s_i+p_i)r_j)}\mathbf{F}_{\mathsf{F}}[q_1^{r_1/2}q_2^{r_2/2}q_3^{r_3/2}] {\mathbf{F}}_a(h_1, h_2, h_3, c)[q_1^{s_1/2}q_2^{s_2/2}q_3^{s_3/2}]
\fe

Using the decomposition of the representation $\mathcal{M}_{\text{NS}}$ under $\mathsf{Vir}\oplus \mathsf{Vir}\subset \mathsf{SCA}_\nu \otimes \mathsf{F}$, we can also express $\widehat{\mathbf{F}}_a(h_1, h_2, h_3, c)$ in terms of Virasoro blocks. The three point function will factorize into  three point function of double virasoro as
\ie
&\widehat\rho_a\!\left(
L_{-M_1}^{(1)}L_{-N_1}^{(2)}v_1^{p_1},
L_{-M_2}^{(1)}L_{-N_2}^{(2)}v_2^{p_2},
L_{-M_3}^{(1)}L_{-N_3}^{(2)}v_3^{p_3}
\right)
\\
&\quad =
\widehat\rho_a(v_1^{p_1},v_2^{p_2},v_3^{p_3})\,
\rho^{(1)}(L_{-M_1}\nu_1,L_{-M_2}\nu_2,L_{-M_3}\nu_3)\,
\rho^{(2)}(L_{-N_1}\nu_1,L_{-N_2}\nu_2,L_{-N_3}\nu_3).
\fe
The all-NS $\mathsf{SCA}_\nu \otimes \mathsf{F}$ block can be expressed as
\ie
...
&\quad \times (-1)^{(2n_1+p_1)(2n_2+p_2)+(2n_1+p_1)(2n_3+p_3)+(2n_2+p_2)(2n_3+p_3)} \\
&\quad \frac{(G_{h_{1,n_1}^{(1)}})^{AA'}(G_{h_{1,n_1}^{(2)}})^{BB'}(G_{h_{2,n_2}^{(1)}})^{CC'}(G_{h_{2,n_2}^{(2)}})^{DD'}(G_{h_{3,n_3}^{(1)}})^{EE'}(G_{h_{3,n_3}^{(2)}})^{FF'}}{\|v^{p_1}_{1,n_1}\|^2\|v^{p_2}_{2,n_2}\|^2\|v^{p_3}_{3,n_3}\|^2}\\
&\quad \times \hat{\rho}_a(L_{-A}^{(1)}L_{-B}^{(2)}v^{p_1}_{1,n_1},L_{-C}^{(1)}L_{-D}^{(2)}v^{p_2}_{2,n_2},L_{-E}^{(1)}L_{-F}^{(2)}v^{p_3}_{3,n_3})\hat{\rho}_a(L_{-A'}^{(1)}L_{-B'}^{(2)}v^{p_1}_{1,n_1},L_{-C'}^{(1)}L_{-D'}^{(2)}v^{p_2}_{2,n_2},L_{-E'}^{(1)}L_{-F'}^{(2)}v^{p_3}_{3,n_3}),
\fe
...
The 3-point function $\hat{\rho}_a(L_{-A}^{(1)}L_{-B}^{(2)}v^{p_1}_{1,n_1},L_{-C}^{(1)}L_{-D}^{(2)}v^{p_2}_{2,n_2},L_{-E}^{(1)}L_{-F}^{(2)}v^{p_3}_{3,n_3})$ can be reduced to those of Virasoro primaries $\hat{\rho}_a(v^{p_1}_{1,n_1}, v^{p_2}_{2,n_2}, v^{p_3}_{3,n_3})$ by conformal Ward identity. The two copies of Virasoro can be reduced independently, which leads to the factorization formula
\ie
\hat{\rho}_a&(L_{-A}^{(1)}L_{-B}^{(2)}v^{p_1}_{1,n_1},L_{-C}^{(1)}L_{-D}^{(2)}v^{p_2}_{2,n_2},L_{-E}^{(1)}L_{-F}^{(2)}v^{p_3}_{3,n_3})\\
&\quad =\hat{\rho}_a(v^{p_1}_{1,n_1}, v^{p_2}_{2,n_2}, v^{p_3}_{3,n_3})\rho(L_{-A}\nu_{h_{1,n_1}^{(1)}}, L_{-C}\nu_{h_{2,n_2}^{(1)}}, L_{-E}\nu_{h_{3,n_3}^{(1)}})\rho(L_{-B}\nu_{h_{1,n_1}^{(2)}}, L_{-D}\nu_{h_{2,n_2}^{(2)}}, L_{-F}\nu_{h_{3,n_3}^{(2)}}),
\fe
where $\rho(L_{-A}\nu_1, L_{-B}\nu_2, L_{-C}\nu_3)$ is defined in section 1. If we define the branching coefficient
\ie
B_a(h_1, h_2, h_3;n_1, n_2, n_3) = \frac{\hat{\rho}_a(v^{p_1}_{1,n_1}, v^{p_2}_{2,n_2}, v^{p_3}_{3,n_3})}{\|v^{p_1}_{1,n_1}\|\|v^{p_2}_{2,n_2}\|\|v^{p_3}_{3,n_3}\|},
\fe
...
&\qquad\times (-1)^{(2n_1+p_1)(2n_2+p_2)+(2n_1+p_1)(2n_3+p_3)+(2n_2+p_2)(2n_3+p_3)}\\
...
\ie
&\mathbf{F}_a(h_1, h_2, h_3, c) = (\mathbf{F}_{\mathsf{F}})^{-1}\star \sum_{\substack{n_1, n_2, n_3\in \frac{1}{2}\mathbb{Z} \\ (-1)^{2(n_1+n_2+n_3)}=(-1)^a}} q_1^{2n_1^2}q_2^{2n_2^2}q_3^{2n_3^2}\eta_1^{2n_1+p_1}\eta_2^{2n_2+p_2}\eta_3^{2n_3+p_3}\\
&\qquad\times (-1)^{(2n_1+p_1)(2n_2+p_2)+(2n_1+p_1)(2n_3+p_3)+(2n_2+p_2)(2n_3+p_3)}\\
&\qquad \times [B_a(h_1,h_2,h_3;n_1,n_2,n_3)]^2 F(h_{1,n_1}^{(1)},h_{2,n_2}^{(1)},h_{3,n_3}^{(1)},c^{(1)})F(h_{1,n_1}^{(2)},h_{2,n_2}^{(2)},h_{3,n_3}^{(2)},c^{(2)}).
\fe
...
\subsubsection*{A Recursion Algorithm For Branching Coefficient}


In this subsection, we explore a recursive algorithm for constructing the double Virasoro primary in the PBW basis. The idea is to study the state $L_1 \ket{w_n}$. In the following, we will prove that $L_1 w_n$ is the double Virasoro descendant of $w_{n-1}$. Namely, it admits an expansion as
\ie
L_1\ket{w_n} =\sum_{|A|+|B|=4n-3} C_{AB} L_{-A}^{(1)} L_{-B}^{(2)}  \ket{w_{n-1}} .
\fe
Meanwhile, following from the free field representation, we can show that the following linear equations uniquely determines $L_1 \ket{w_n}$ up to a constant:
\ie
J_{-\mu} L_1 \ket{w_n}= 2^{\ell(\mu)} \ket{w_{n-1}},\quad  c_m L_1 \ket{w_n}= 0.
\fe
It allows us to pin down the coefficient $C_{AB}$.

To prove the relations above, introduce
\ie
\chi(z)= \frac{1}{\sqrt{2}}\left(\psi(z)-i\eta(z)\right),~ \chi(z)^*= \frac{1}{\sqrt{2}}\left(\psi(z)+i\eta(z)\right).
\fe
The $U(1)$ current for the complex fermion is given by
\ie
J(z) =\frac{1}{2} : \chi(z)\chi^*{z}:
\fe
with mode expansion
\ie
J(z)=\sum_{n} J_{n} z^{-n-1}, \quad J_{n}=\frac{1}{2}\sum_{r\in \mathbb{Z}+\frac{1}{2}} :\chi_{n-r}\chi^*_{r}:.
\fe
It satisfy
\ie
[J_m,J_n] = m\delta_{m+n,0}, \quad [J_m,\chi_r] = \chi_r,\quad [J_m,\chi^*_r] = -\chi^*_r.
\fe
...
\ie
\braket{ L_{-A'}^{(1)} L_{-B'}^{(2)}  w_{n-1}}{L_1 w_n} = \braket{L_{-1} L_{-A'}^{(1)} L_{-B'}^{(2)}  w_{n-1}}{w_n}
\fe
...
\ie
\braket{ L_{-A'}^{(1)} L_{-B'}^{(2)}  w_{n-1}}{L_1 w_n} =\sum_{|A|+|B|=4n-3} C_{AB} ||w_{n-1}||^2 \mathbb{G}_{AA'} \mathbb{G}_{BB'}
\fe


Combine the fermion $\psi, \eta$ into one complex fermion $ \chi$, with
\ie
 \chi_{r} =\frac{1}{\sqrt{2}}( \psi_r - i\eta_r), \quad  \chi^*_{r} =\frac{1}{\sqrt{2}}( \psi_r +i\eta_r).
\fe
They will satisfy the commutation relation
\ie
\left\{ \chi_{r},\chi^*_{s}\right\} = \delta_{r+s,0}.
\fe
In the free field representation, the double virasoro primary can be represented by
\ie
w_n(P)=\chi_{-\frac{4n-1}{2}}(P)\chi_{-\frac{4n-3}{2}}(P)\cdots \chi_{-\frac{1}{2}}(P)\phi(P).
\fe
...
&=(-1)^{A\mathsf{A}+(B+|\alpha|)(\mathsf{C+c})+p_1(\mathsf{C+c})}\rho(\Psi_{-\mathsf{A}}\mathbf{1}_{\mathsf{F}}, \boldsymbol\Psi_{-\mathsf{B}}u^{\mathsf{b}},\boldsymbol\Psi_{-\mathsf{C}}u^{\mathsf{c}}) {\rho}_f^{(\eta)}(\mathbf{L}_{-A}\phi_1, \mathbb{L}_{-B}w^\alpha_2, \mathbb{L}_{-C}w^\gamma_3),
\fe
where the fermionic 3-point function is normalized such that $\rho(\mathbf{1}_{\mathsf{F}}, u^0, u^0)=1,\rho(\mathbf{1}_{\mathsf{F}}, u^1, u^1)=i$. Therefore, if we define the fermion Ramond block as
\ie
\mathbb{F}_{\mathsf{F}} = \sum_{\mathsf{A+B+C+b+c}\equiv 0 \mod 2} &(-1)^{\mathsf{A}}q_1^{|\mathsf{A}|} \eta_1^{\mathsf{A}} q_2^{|\mathsf{B}|} \eta_2^{\mathsf{B}+\mathsf{b}} q_3^{|\mathsf{C}|} \eta_3^{\mathsf{C}+\mathsf{c}} (-1)^{\mathsf{A(B+b)+A(C+c)+(B+b)(C+c)}}\\
...
\widehat{\mathbb{F}}^{(\eta, \eta')}_f&(h_1, \beta_2, \beta_3, c) = \sum_{\substack{
  ...
  }} q_1^{2n_1^2+|A|+|B|}q_2^{2n_2^2-\frac{1}{8}+|C|+|D|}q_3^{2n_3^2-\frac{1}{8}+|E|+|F|}\eta_1^{2n_1+p_1}\eta_2^{\alpha_2}\eta_3^{\alpha_3}\\
&\times (-1)^{(2n_1+p_1)\alpha_2+(2n_1+p_1)\alpha_3+\alpha_2\alpha_3} \frac{(G_{h_{1,n_1}^{(1)}})^{AA'}(G_{h_{1,n_1}^{(2)}})^{BB'}(G_{h_{2,n_2}^{(1)}})^{CC'}(G_{h_{2,n_2}^{(2)}})^{DD'}(G_{h_{3,n_3}^{(1)}})^{EE'}(G_{h_{3,n_3}^{(2)}})^{FF'}}{\|v^{p_1}_{1,n_1}\|^2\|v_{2,n_2}^{\alpha_2}\|^2\|v_{3,n_3}^{\alpha_3}\|^2}\\
&\times \hat{\rho}_f^{(\eta)}(L_{-A}^{(1)}L_{-B}^{(2)}v^{p_1}_{1,n_1},L_{-C}^{(1)}L_{-D}^{(2)}v_{2,n_2}^{\alpha_2},L_{-E}^{(1)}L_{-F}^{(2)}v_{3,n_3}^{\alpha_3})\hat{\rho}_f^{(\eta')}(L_{-A'}^{(1)}L_{-B'}^{(2)}v^{p_1}_{1,n_1},L_{-C'}^{(1)}L_{-D'}^{(2)}v_{2,n_2}^{\alpha_2},L_{-E'}^{(1)}L_{-F'}^{(2)}v_{3,n_3}^{\alpha_3}),
\fe
...
\ie
\hat{\rho}_f^{(\eta)}&(L_{-A}^{(1)}L_{-B}^{(2)}v^{p_1}_{1,n_1},L_{-C}^{(1)}L_{-D}^{(2)}v_{2,n_2}^{\alpha_2},L_{-E}^{(1)}L_{-F}^{(2)}v_{3,n_3}^{\alpha_3})\\
&\quad =\hat{\rho}_f^{(\eta)}(v^{p_1}_{1,n_1}, v_{2,n_2}^{\alpha_2}, v_{3,n_3}^{\alpha_3})\rho(L_{-A}\nu_{h_{1,n_1}^{(1)}}, L_{-C}\nu_{h_{2,n_2}^{(1)}}, L_{-E}\nu_{h_{3,n_3}^{(1)}})\rho(L_{-B}\nu_{h_{1,n_1}^{(2)}}, L_{-D}\nu_{h_{2,n_2}^{(2)}}, L_{-F}\nu_{h_{3,n_3}^{(2)}}),
\fe
and therefore,
\ie
\widehat{\mathbb{F}}_f^{(\eta,\eta')}(h_1, \beta_2, \beta_3, c) &= \sum_{\substack{n_1 \in \frac{1}{2} \mathbb{Z}, n_2, n_3\in \frac{1}{2}\mathbb{Z}+\frac{1}{4}\\(-1)^{2n_1+\alpha_2+\alpha_3}=(-1)^f}} q_1^{2n_1^2}q_2^{2n_2^2-\frac{1}{8}}q_3^{2n_3^2-\frac{1}{8}}\eta_1^{2n_1+p_1}\eta_2^{\alpha_2}\eta_3^{\alpha_3}\\
&\quad \times (-1)^{2n_1+(2n_1+p_1)\alpha_2+(2n_1+p_1) \alpha_3+\alpha_2\alpha_3} \\
&\quad \times\mathbb{B}^{(\eta);p_1}_f(h_1, \beta_2, \beta_3;n_1, n_2, n_3) \mathbb{B}^{(\eta');p_1}_f(h_1, \beta_2, \beta_3;n_1, n_2, n_3)\\
...
\ie
\mathbb{B}^{(\eta);p_1}_f(h_1, \beta_2, \beta_3;n_1, n_2, n_3) = \frac{\hat{\rho}_f^{(\eta)}(v^{p_1}_{1,n_1}, v_{2,n_2}^{\alpha_2}, v_{3,n_3}^{\alpha_3})}{\|v^{p_1}_{1,n_1}\|\|v_{2,n_2}^{\alpha_2}\|\|v_{3,n_3}^{\alpha_3}\|}.
\fe
where $v^{p_1}_{1,n_1}$ has parity $2n_1+p_1$.

\subsubsection*{Ramond branching coefficients}

Unlike the NS branching coefficient $\mathbb{B}_a$, the Ramond branching coefficient $\mathbb{B}_f^{(\eta);p_1}$ is not computed in the literature. There are conjectures in \cite{1312.4520} about its structure, but the numerical prefactor remains unknown. In the rest of this section, we develop a recursion relation between $\mathbb{B}^{(\eta);p_1}_f(n_1, n_2, n_3, \alpha_2, \alpha_3)$'s, which can be used to solve them numerically.

Let's first define the Virasoro primaries $v_n^\alpha$ unambiguously. Similar to the NS algebra, one can also introduce free bosons $c_n$ and free fermions $\eta_r$ with $n, r\in \mathbb{Z}$ to realize the $\mathsf{SCA}_0$ algebra. The SCA generators are given by the same formula \eqref{freefieldrep}. Denoting $\eta_r(P)$ as the operator $\eta_r$ expressed using $G_r, L_n, P$. Using $\chi_r(P) = \psi_r - i\eta_r(P)$, we define $v_n^\alpha$ with $n>0$ as
\ie
...
\fe
where the subscripts $\pm$ denote the components ending in \(w_\beta^+\) and \(w_\beta^-\), respectively. The BPZ norms of the Virasoro primaries can be computed by
\ie
\left\|v_n^0(P)\right\|^2 & =2^{2\lfloor M/2 \rfloor+1} \frac{\ell(2 P, 4 n)}{\ell(Q+2 P, 4 n)}, \\
\left\|v_n^1(P)\right\|^2 & =-2^{2\lceil M/2 \rceil} \frac{\ell(2 P, 4 n)}{\ell(Q+2 P, 4 n)},
\fe
where $M=2n-\frac{1}{2}$.

The key observation that leads to the recursion relation is the following:
\begin{proposition}
For Ramond $\mathsf{Vir\oplus Vir}$ primaries with $n\geq \frac{3}{4}$, $L_{1} v_n^\alpha$ only contains level-$(4n-3)$ $\mathsf{Vir\oplus Vir}$ descendants of $v_{n-1}^\alpha$, and $L_{-1} v_n^\alpha$ only contains level-1 $\mathsf{Vir\oplus Vir}$ descendants of $v_n^\alpha$ and level-$(4n-1)$ $\mathsf{Vir\oplus Vir}$ descendants of $v_{n-1}^\alpha$. Namely,
\ie
...
\begin{proposition}
For NS $\mathsf{Vir\oplus Vir}$ primaries with $n\geq 1$, $L_{1} v_n$ only contains level-$(4n-3)$ $\mathsf{Vir\oplus Vir}$ descendants of $v_{n-1}$, and $L_{-1} v_n$ only contains level-1 $\mathsf{Vir\oplus Vir}$ descendants of $v_n$ and level-$(4n-1)$ $\mathsf{Vir\oplus Vir}$ descendants of $v_{n-1}$. Namely,
\ie
...
\ie
\hat{\rho}_f^{(\eta)}(L_1v^{p_1}_{1,n_1},v_{2,n_2}^{\alpha_2},v_{3,n_3}^{\alpha_3})&=\hat{\rho}_f^{(\eta)}(v^{p_1}_{1,n_1},L_{-1}v_{2,n_2}^{\alpha_2},v_{3,n_3}^{\alpha_3})+\hat{\rho}_f^{(\eta)}(v^{p_1}_{1,n_1},v_{2,n_2}^{\alpha_2},L_{-1}v_{3,n_3}^{\alpha_3}),\\
\hat{\rho}_f^{(\eta)}(v^{p_1}_{1,n_1},L_1v_{2,n_2}^{\alpha_2},v_{3,n_3}^{\alpha_3})&=\hat{\rho}_f^{(\eta)}(L_{-1}v^{p_1}_{1,n_1},v_{2,n_2}^{\alpha_2},v_{3,n_3}^{\alpha_3})-\hat{\rho}_f^{(\eta)}(v^{p_1}_{1,n_1},L_{-1}v_{2,n_2}^{\alpha_2},v_{3,n_3}^{\alpha_3})\\
&\quad -2\hat{\rho}_f^{(\eta)}(v^{p_1}_{1,n_1},L_0v_{2,n_2}^{\alpha_2},v_{3,n_3}^{\alpha_3})-\hat{\rho}_f^{(\eta)}(v^{p_1}_{1,n_1},v_{2,n_2}^{\alpha_2},L_1v_{3,n_3}^{\alpha_3}). 
\fe
Plugging in the decomposition above, the first equation then reads
\ie
&\sum_{|A|+|B|=4n_1-3}\mathbf{V}_{1,n_1}^{AB}\rho(L_{-A}\nu_{h_{1,n_1-1}^{(1)}}, \nu_{h_{2,n_2}^{(1)}}, \nu_{h_{3,n_3}^{(1)}})\rho(L_{-B}\nu_{h_{1,n_1-1}^{(2)}}, \nu_{h_{2,n_2}^{(2)}}, \nu_{h_{3,n_3}^{(2)}})\hat{\rho}_f^{(\eta)}(v_{1,n_1-1},v_{2,n_2}^{\alpha_2},v_{3,n_3}^{\alpha_3})\\
&\qquad = \left[\sum_{i=1}^2 (\mathbb{V}_{n_2,\alpha_2}^{(i)}-\mathbb{V}_{n_3,\alpha_3}^{(i)})(h_{1,n_1}^{(i)}-h_{2,n_2}^{(i)}-h_{3,n_3}^{(i)})\right]\hat{\rho}_f^{(\eta)}(v^{p_1}_{1,n_1}, v_{2, n_2}^{\alpha_2}, v_{3, n_3}^{\alpha_3})\\
&\qquad + \sum_{|C|+|D|=4n_2-1} \mathbb{V}_{-1,n_2, \alpha_2}^{CD}\rho(\nu_{h_{1,n_1}^{(1)}},L_{-C}\nu_{h_{2,n_2-1}^{(1)}}, \nu_{h_{3,n_3}^{(1)}})\rho(\nu_{h_{1,n_1}^{(2)}},L_{-D}\nu_{h_{2,n_2-1}^{(2)}}, \nu_{h_{3,n_3}^{(2)}})\hat{\rho}_f^{(\eta)}(v^{p_1}_{1,n_1},v_{2,n_2-1}^{\alpha_2},v_{3,n_3}^{\alpha_3})\\
&\qquad + \sum_{|E|+|F|=4n_3-1} \mathbb{V}_{-1,n_3, \alpha_3}^{EF}\rho(\nu_{h_{1,n_1}^{(1)}},\nu_{h_{2,n_2}^{(1)}}, L_{-E}\nu_{h_{3,n_3-1}^{(1)}})\rho(\nu_{h_{1,n_1}^{(2)}},\nu_{h_{2,n_2}^{(2)}}, L_{-F}\nu_{h_{3,n_3-1}^{(2)}})\hat{\rho}_f^{(\eta)}(v^{p_1}_{1,n_1},v_{2,n_2}^{\alpha_2},v_{3,n_3-1}^{\alpha_3}),
\fe
where we have used the fact that $\rho(\nu_1,L_{-1}\nu_2,\nu_3)=h_1-h_2-h_3,\rho(\nu_1,\nu_2,L_{-1}\nu_3)=-h_1+h_2+h_3$ and the factorization of 3-point functions. Therefore,
{\footnotesize
\ie
&\left[\sum_{i=1}^2 (\mathbb{V}_{n_2,\alpha_2}^{(i)}-\mathbb{V}_{n_3,\alpha_3}^{(i)})(h_{2,n_2}^{(i)}+h_{3,n_3}^{(i)}-h_{1,n_1}^{(i)})\right]\mathbb{B}_f^{(\eta);p_1}(n_1,n_2,n_3,\alpha_2,\alpha_3)=\\
& - \sum_{|A|+|B|=4n_1-3}\mathbf{V}_{1,n_1}^{AB}\rho(L_{-A}\nu_{h_{1,n_1-1}^{(1)}}, \nu_{h_{2,n_2}^{(1)}}, \nu_{h_{3,n_3}^{(1)}})\rho(L_{-B}\nu_{h_{1,n_1-1}^{(2)}}, \nu_{h_{2,n_2}^{(2)}}, \nu_{h_{3,n_3}^{(2)}})\mathbb{B}_f^{(\eta);p_1}(n_1-1,n_2,n_3,\alpha_2,\alpha_3)\frac{\|v_{n_1-1}\|}{\|v_{n_1}\|}\\
& + \sum_{|C|+|D|=4n_2-1} \mathbb{V}_{-1,n_2, \alpha_2}^{CD}\rho(\nu_{h_{1,n_1}^{(1)}},L_{-C}\nu_{h_{2,n_2-1}^{(1)}}, \nu_{h_{3,n_3}^{(1)}})\rho(\nu_{h_{1,n_1}^{(2)}},L_{-D}\nu_{h_{2,n_2-1}^{(2)}}, \nu_{h_{3,n_3}^{(2)}})\mathbb{B}_f^{(\eta);p_1}(n_1,n_2-1,n_3,\alpha_2,\alpha_3)\frac{\|v_{n_2-1}^{\alpha_2}\|}{\|v_{n_2}^{\alpha_2}\|}\\
& + \sum_{|E|+|F|=4n_3-1} \mathbb{V}_{-1,n_3, \alpha_3}^{EF}\rho(\nu_{h_{1,n_1}^{(1)}},\nu_{h_{2,n_2}^{(1)}}, L_{-E}\nu_{h_{3,n_3-1}^{(1)}})\rho(\nu_{h_{1,n_1}^{(2)}},\nu_{h_{2,n_2}^{(2)}}, L_{-F}\nu_{h_{3,n_3-1}^{(2)}})\mathbb{B}_f^{(\eta);p_1}(n_1,n_2,n_3-1,\alpha_2,\alpha_3)\frac{\|v_{n_3-1}^{\alpha_3}\|}{\|v_{n_3}^{\alpha_3}\|},
\fe
}
and the ratios can be computed by
\ie
\frac{\left\|v_{n_1-1}\right\|}{\left\|v_{n_1}\right\|}=2 \sqrt{\frac{\ell\left(2 P_1, 4 n_1-4\right) \ell\left(Q+2 P_1, 4 n_1-4\right)}{\ell\left(2 P_1, 4 n_1\right) \ell\left(Q+2 P_1, 4 n_1\right)}},\quad \frac{\left\|v_{n_2-1}^{a_2}\right\|}{\left\|v_{n_2}^{a_2}\right\|}=\frac{1}{2} \sqrt{\frac{\ell\left(2 P_2, 4 n_2-4\right) \ell\left(Q+2 P_2, 4 n_2\right)}{\ell\left(Q+2 P_2, 4 n_2-4\right) \ell\left(2 P_2, 4 n_2\right)}}.
\fe
Using this recursion relation, one can solve for the Ramond branching coefficient $\mathbb{B}_f^{(\eta);p_1}(n_1,n_2,n_3,\alpha_2,\alpha_3)$ recursively. 
One should note that this form only works for $n_1\geq 1$ and $n_2,n_3\geq \frac{3}{4}$. At the boundary values, $L_1$ annihilates $v_{0,\pm \frac{1}{2}}$ and $v_{\pm \frac{1}{4}}^{\alpha}$. For $L_{-1}$, one can use 
\ie
L_{-1}v_0 &= L_{-1}^{(1)}v_0 + L_{-1}^{(2)}v_0\\
L_{-1} v_{1 / 2}= & -\frac{(2 P+b)\left(-2 P b+b^2+1\right)}{2 P\left(2 P b+b^2-1\right)} L_{-1}^{(1)} v_{1 / 2}+\frac{(2 P b+1)\left(-2 P b+b^2+1\right)}{2 P b\left(-2 P b+b^2-1\right)} L_{-1}^{(2)} v_{1 / 2} \\
& +\frac{b\left(2 P b+b^2+1\right)}{2 P\left(-2 P b+b^2-1\right)} L_{-1}^{(1)} v_{-1 / 2}-\frac{2 P b+b^2+1}{2 P b\left(2 P b+b^2-1\right)} L_{-1}^{(2)} v_{-1 / 2}, \\
L_{-1} v_{-1 / 2}= & -\frac{b\left(-2 P b+b^2+1\right)}{2 P\left(2 P b+b^2-1\right)} L_{-1}^{(1)} v_{1 / 2}+\frac{-2 P b+b^2+1}{2 P b\left(-2 P b+b^2-1\right)} L_{-1}^{(2)} v_{1 / 2} \\
& +\frac{(-2 P+b)\left(2 P b+b^2+1\right)}{2 P\left(-2 P b+b^2-1\right)} L_{-1}^{(1)} v_{-1 / 2}+\frac{(2 P b-1)\left(2 P b+b^2+1\right)}{2 P b\left(2 P b+b^2-1\right)} L_{-1}^{(2)} v_{-1 / 2}.\\
L_{-1} v_{1 / 4}^\alpha= & -\frac{-4 P b+b^2+2}{2(2 P b-1)} L_{-1}^{(1)} v_{1 / 4}^\alpha+\frac{-4 P b+2 b^2+1}{2 b(-2 P+b)} L_{-1}^{(2)} v_{1 / 4}^\alpha \\
& -\frac{\left(-2 P b+b^2+2\right)\left(-2 P b+2 b^2+1\right)}{2 a+2 b(-2 P+b)(2 P b-1)} v_{-3 / 4}^\alpha \\
L_{-1} v_{-1 / 4}^\alpha= & \frac{4 P b+b^2+2}{2(2 P b+1)} L_{-1}^{(1)} v_{-1 / 4}^\alpha+\frac{4 P b+2 b^2+1}{2 b(2 P+b)} L_{-1}^{(2)} v_{-1 / 4}^\alpha \\
& +\frac{\left(2 P b+b^2+2\right)\left(2 P b+2 b^2+1\right)}{2^{\alpha+2} b(2 P+b)(2 P b+1)} v_{3 / 4}^\alpha .
\fe
Note that the $L_{-1}$ action on $v_{1/4}$ involves $v_{-3/4}$. One can use the similar reduction relation for $L_{\pm 1}$ acting on negative $n$ to deal with it. It will turns out that $L_{-1} v_{-3/4}$ only contains level-1 descendant of $v_{-3/4}$ and level-2 descendants of $v_{1/4}$, while $L_{1}v^{\alpha}_{-3/4}$ is proportional to $v_{1/4}$. In the end, one only needs to compute a few low-level branching coefficients directly for $n_{\NS} = 0,\pm \frac{1}{2}$ and $n_{\R} = \pm \frac{1}{4}, \pm \frac{3}{4}$, and every other branching coefficient can be obtained using the recursion relation. Using this method, branching coefficients $\mathbb{B}_f^{(\eta')}(2,\frac{7}{4}, \frac{5}{4},\alpha_2, \alpha_3)$ with all 8 sign configurations takes around 1 second in total on a laptop. The results are also checked against low-level direct calculations for NS $|n|\leq \frac{3}{2}$ and Ramond $|n|\leq \frac{5}{4}$.
...
Using the branching coefficients, one can compute the full $\mathsf{SCA}_0\otimes \mathsf{F}_0$ block using \eqref{rextendedblock}. We have computed this quantity in two different ways: First by the double Virasoro decomposition, and then by directly computing the blocks $\mathbb{F}_\mathsf{F}$ and $\mathbb{F}_f^{(\eta, \eta')}$ by summing over descendants. We have checked that the two blocks agrees up to order $q^6$. Computing the block $\hat{\mathbb{F}}^{(+,+)}_0$ as a $q$-expansion takes 83 seconds on a laptop, in which 75 seconds are used to compute the branching coefficients.
