...
\usepackage{amsmath,amssymb,bm,epsf,epsfig,graphicx,hyperref,physics, cleveref, subfig, slashed, stmaryrd, tensor, mathrsfs,}
...
\section{Two copies of Virasoro from SCA}

We now turn to another approach of computing the superconformal blocks. The idea is: Take the $\mathcal{N}=1$ SCA with arbitrary shift $\nu$ in the fermionic levels, tensor it with a auxiliary free fermion algebra $\mathsf{F}$, there will then be a subalgebra $\mathsf{Vir}\oplus \mathsf{Vir}\subset \mathsf{SCA}_\nu \otimes \mathsf{F}$ \cite{1312.4520,1608.02568}. Using the branching rules of this subalgebra reduction, one can decompose the superconformal block into ordinary Virasoro blocks.

The enlarged algebra $\mathsf{SCA}_\nu \otimes \mathsf{F}$ is given by:
\ie
{\left[L_m, L_n\right] } & =(m-n) L_{m+n}+\frac{c}{12}\left(m^3-m\right) \delta_{m+n, 0}, \\
...
\end{aligned}
\fe
which satisfies $L_n^{(1)}+L_n^{(2)}=L_n+L_n^{\mathrm{F}}$. Note that in order to have $(L_n^{(i)})^\dagger = L_{-n}^{(i)}$, we take the BPZ conjugate of $\psi_r$ to be $(\psi_r)^\dagger = -\psi_{-r}$. One can also prove that these operators are the generators of two commuting copies of the Virasoro algebra
\ie
[L_m^{(i)}, L_n^{(j)}] = \left[(m-n) L_{m+n} + \frac{c^{(i)}}{12} (m^3-m)\delta_{m+n,0}\right]\delta_{ij},
\fe
...
\fe
This proves the subalgebra relation $\mathsf{Vir}\oplus \mathsf{Vir}\subset \mathsf{SCA}_\nu \otimes \mathsf{F}$. Under the subalgebra reduction $\mathsf{Vir}\oplus \mathsf{Vir}\subset \mathsf{SCA}_\nu \otimes \mathsf{F}$, a highest-weight representation of $\mathsf{SCA}_\nu \otimes \mathsf{F}$ can be decomposed into irreps of $\mathsf{Vir}\oplus \mathsf{Vir}$. This decomposition is worked out in \cite{1608.02568}. We now discuss this decomposition in both NS and R sectors.

As always, we denote a NS sector primary of $\mathsf{SCA}_{1/2}$ by $\phi$, with its conformal weight denoted by $h$. We further define the "Liouville momentum" $P$ by
\ie
h=\frac{1}{2}\left(\frac{1}{4}Q^2-P^2\right),
\fe
where $Q=b+b^{-1}$. The space $\mathcal{M}_{\mathrm{NS}}(h)$ is then defined by acting arbitrary $\psi_{-r}, L_{-n}, G_{-r}$ on $\mathbf{1}_{\mathsf{F}}\otimes \phi$. Denoting the Virasoro Verma module with lowest weight $h$ by $\mathcal{V}_c(h)$, it's proven that \cite{1111.2803}
...
\fe
for $P\notin \frac{1}{2}(b\mathbb{Z}+b^{-1}\mathbb{Z})$. The weights $h^{(i)}_{n}$'s are given by $h^{(i)}_n = \frac{1}{4}(Q^{(i)})^2-(P_{n}^{(i)})^2$
\ie \label{branchweights}
P_n^{(1)}=\frac{P}{\sqrt{2-2 b^2}}+n b^{(1)},\quad P_n^{(2)}=\frac{P}{\sqrt{2-2 b^{-2}}}+n(b^{(2)})^{-1}.
\fe
We will denote the highest weight vector in $\mathcal{V}_{c^{(1)}}(h_{n}^{(1)}) \otimes \mathcal{V}_{c^{(2)}}(h_{n}^{(2)})$ by $v_{n}$. In our convention where $\phi$ is Grassmann even, $v_n$ has Grassmann parity $2n$.

In the R sector, due to the existence of the fermion zero mode $\psi_0$, there exists two ground states $u^\pm$ in the $\mathsf{F}$ module, with $\psi_0$ action $\psi_0 u^\pm = 2^{-1/2}u^\mp$. The $\mathsf{SCA}_0\otimes \mathsf{F}$ module contains 4 ground states given by $u^\pm \otimes w^\pm_\beta$, where $w^\pm_\beta$ is a R ground state as always. The Liouville momentum of $w_\beta^\pm$ is defined by
\ie
h = \frac{c}{24}-\beta^2 = \frac{1}{16} + \frac{1}{2}\left(\frac{1}{4}Q^2-P^2\right),\quad P^2 = 2\beta^2.
\fe
Defining the space $\mathcal{M}_{\R}(\beta)$ by acting lowering operators on the 4 ground states $u^\pm \otimes w_\beta^\pm$. For $P\notin \frac{1}{2}(b\mathbb{Z}+b^{-1}\mathbb{Z})$ it can be decomposed into \cite{1608.02568}
\ie\label{Rbranching}
...
& \times (-1)^{AB+AC+BC+\mathsf{AB}+\mathsf{AC}+\mathsf{BC}}(-1)^{\mathsf{A}+\mathsf{B}+\mathsf{C}}q_1^{|\mathsf{A}|}\eta_1^{\mathsf{A}}q_2^{|\mathsf{B}|}\eta_2^{\mathsf{B}}q_3^{|\mathsf{C}|} \eta_3^{\mathsf{C}}\\
...
\fe
where ${\Psi}_{-\mathsf{A}}v$ stands for $\Psi_{-\mathsf{A}}v \equiv \psi_{-r_1}\cdots \psi_{-r_m} v$, $|\mathsf{A}|$ stands for the total level of fermion lowering operators, and $(-1)^{\mathsf{A}}$ is the total $\psi$ parity. We have used the fact that $\langle \mathbf{L}_{-A}v| \Psi_{-\mathsf{A}}^\dagger \Psi_{-\mathsf{B}}|\mathbf{L}_{-A}v\rangle$ is the matrix $(-1)^{\mathsf{A}}\delta_{\mathsf{A},\mathsf{B}}\mathbf{G}_{AB}$, which give rises to the sign $(-1)^{\mathsf{A}+\mathsf{B}+\mathsf{C}}$. The 3-point function $\hat{\rho}^{L/R}$ is labeled by the total parity, including the fermionic one.\footnote{The sign $(-1)^{AB+AC+BC+\mathsf{AB}+\mathsf{AC}+\mathsf{BC}}$ arises from the moving the operator order from $\mathbf{L}_{-C}\Psi_{-\mathsf{C}}\phi_3\mathbf{L}_{-B}\Psi_{-\mathsf{B}}\phi_2\mathbf{L}_{-A}\Psi_{-\mathsf{A}}\phi_1\Psi_{-\mathsf{A}}\mathbf{L}_{-A'}\phi_1\Psi_{-\mathsf{B}}\mathbf{L}_{-B'}\phi_2\Psi_{-\mathsf{C}}\mathbf{L}_{-C'}\phi_3$ to the order in the 3-point functions. We have used the fact $\mathsf{A}+\mathsf{B}+\mathsf{C}$ is always even.}
...
\fe
where $\rho^{\mathsf{F}}(u_1, u_2, u_3)$ is the 3-point function in the free fermion theory. Importantly, $\rho^{\mathsf{F}}$ is only nonzero if $\mathsf{A}+\mathsf{B}+\mathsf{C}\equiv 0$ mod 2. If we define the free fermion block as
\ie
\mathbf{F}^{\mathsf{F}}(q_i, \eta_i) =& \sum_{\substack{\mathsf{A}, \mathsf{B}, \mathsf{C}\\(-1)^{\mathsf{A}+\mathsf{B}+\mathsf{C}}=1}}(-1)^{\mathsf{AB}+\mathsf{AC}+\mathsf{BC}}(-1)^{\mathsf{A}+\mathsf{B}+\mathsf{C}}q_1^{|\mathsf{A}|}\eta_1^{\mathsf{A}}q_2^{|\mathsf{B}|}\eta_2^{\mathsf{B}}q_3^{|\mathsf{C}|} \eta_3^{\mathsf{C}}\left[{\rho}^{\mathsf{F}}(\Psi_{-\mathsf{A}}\mathbf{1}_\mathsf{F}, \Psi_{-\mathsf{B}}\mathbf{1}_\mathsf{F}, \Psi_{-\mathsf{C}}\mathbf{1}_\mathsf{F})\right]^2,
\fe
then the all-NS $\mathsf{SCA}_\nu \otimes \mathsf{F}$ block can be factorized into
\ie
\widehat{\mathbf{F}}_a(h_1, h_2, h_3, c;q_i, \eta_i)=\mathbf{F}_{\mathsf{F}}(q_i, \eta_i)\times {\mathbf{F}}_a(h_1, h_2, h_3, c;q_i, \eta_i).
\fe
...
\fe
where $F(h_1, h_2, h_3, c)$ is the Virasoro conformal block, which can be computed efficiently using the $c$-relation relation in section 1. Since the fermion block $\mathbb{F}_{\mathsf{F}}$ will always have the identity term $1$, one can invert $\mathbb{F}_{\mathsf{F}}$ as a formal power series in $q$. The all-NS superconformal block $\mathbf{F}_a$ can then be computed as
\ie
&\mathbf{F}_a(h_1, h_2, h_3, c) = (\mathbf{F}_{\mathsf{F}})^{-1}\sum_{\substack{n_1, n_2, n_3\in \frac{1}{2}\mathbb{Z} \\ (-1)^{2(n_1+n_2+n_3)}=(-1)^a}} q_1^{2n_1^2}q_2^{2n_2^2}q_3^{2n_3^2}\eta_1^{2n_1}\eta_2^{2n_2}\eta_3^{2n_3}(-1)^{4n_1n_2+4n_1n_3+4n_2n_3}\\
&\qquad \times [B_a(h_1,h_2,h_3;n_1,n_2,n_3)]^2 F(h_{1,n_1}^{(1)},h_{2,n_2}^{(1)},h_{3,n_3}^{(1)},c^{(1)})F(h_{1,n_1}^{(2)},h_{2,n_2}^{(2)},h_{3,n_3}^{(2)},c^{(2)}).
\fe

The remaining problem is to compute the branching coefficient $B_a(h_1,h_2,h_3;n_1,n_2,n_3)$. This can be done using the free field representation in \cite{1608.02568}.
