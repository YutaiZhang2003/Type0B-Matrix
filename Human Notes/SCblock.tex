...
\fe
The holomorphic and anti-holomorphic generators mutually commute.
...
In this section, we only consider the case where all puncture are NS, where the $c$-recursion relation is more or less parallel to the Virasoro case. The NS sector supervirasoro algebra is given by 
...
\fe
where $n_1 \geq ... \geq n_\ell, r_1>...>r_p$. We will also take $\phi$ to be Grassmann even. \todo[inline]{Yutai: I do not know why we can choose the parity of phi in general. I would assume it to be generic.} In the language of 2D $\mathcal{N}=1$ superspace, one can pack $\phi$ and its level-$\frac{1}{2}$ descendent $\chi \equiv G_{-1/2}\phi$ in the primary superfield $\Phi(z,\theta)$, defined by
\ie
...
For illustration, we consider a 2d \(\mathcal N=(1,1)\) superconformal field theory and derive the conformal-block decomposition of its genus-two partition function. We work in the theta-graph plumbing channel, taking all three tubes to have (NS,NS) spin structure in both the holomorphic and anti-holomorphic sectors. As we will see, the fermionic nature of the super-Virasoro generators \(G_r\) introduces additional signs that obstruct a naive conformal-block decomposition.
...
\fe
The parity of $\mathcal{O}_i$ is $p_i+\tilde{p}_i$. 

In the theta plumbing channel, the genus-two partition function \(Z_{g=2}\) has a prescription
\ie
Z_{g=2}
={}&
...
\prod_{i=1}^{3}
q_i^{h_i}\widetilde q_i^{\widetilde h_i}
\times
q_1^{|A|}\eta_1^A
q_2^{|C|}\eta_2^C
q_3^{|E|}\eta_3^E
...
\right\rangle\right.
\fe
The sign in the sum comes from reordering the operators into the prescription of the matrix element. The gram matrix is defined by
\ie
...
(-1)^{(\widetilde A+ \tilde{p}_1)  (B+p_1)+ p_1\tilde{p}_1}
(\mathbf{G}_{h_i})_{AB}
...
\right).
\fe
Here $A+C+E+p_1+p_2+p_3=\tilde{A} +\tilde{C}+\tilde{E}+\tilde{p}_1+\tilde{p}_2+\tilde{p}_3=a~\text{mod} 2$. In this step, the sign comming from the reordering of holomorphic primaries and anti-holomorphic primaries will cancel out because the same factor appears in both matrix element.
...
\fe
and analogously the anti-holomorphic block $\widetilde{F}_a$. The genus 2 partition function then reduces to
\ie
Z_{g=2} = \sum_{\mathcal{O}_1,\mathcal{O}_2,\mathcal{O}_3} \sum_{a=0,1}  \left(C^{(a)}_{123} \right)^2(-1)^a \prod_{i=1}^{3}
q_i^{h_i}\widetilde q_i^{\widetilde h_i}F_a(h_1, h_2, h_3, c) \widetilde{F}_a (\tilde{h}_1,\tilde{h}_2, \tilde{h}_3, \tilde{c}).
\fe

\subsubsection*{c-Recursion for NS Block}
We will keep using the theta channel as an example, with the plumbing data denoted by $(q_i, \eta_i)$. For simplicity, we will assume the holomorphic primary operators are grassmann even. The all-NS conformal block is given by
\ie \label{NSblock}
\mathbf{F}_a(h_1, h_2, h_3, c)= & \sum_{|A|=|B|,|C|=|D|,|E|=|F|, (-1)^{A+C+E}=(-1)^a} q_1^{|A|}\eta_1^A q_2^{|C|} \eta_2^C q_3^{|E|} \eta_3^E \mathbf{G}_{h_1}^{A B} \mathbf{G}_{h_2}^{C D} \mathbf{G}_{h_3}^{E F} \\
...
The standard bilinear pairing on $\mathcal{M}_{\mathrm{NS}}(h)$ in the theory of chiral fermion coupled to superconformal field theory is given by
\ie
...
\fe
Introduce the parity operator $(-1)^{p_{\mathsf{SCA}} \cdot p_{\mathsf{F}}}$. We have
\ie
\bpzbraket{\mathbf{\Psi}_{-\mathsf{A}}\mathbf{1}_F \otimes \mathbf{L}_{-A} \phi }{(-1)^{p_{\mathsf{SCA}} \cdot p_{\mathsf{F}}}\mathbf{\Psi}_{-\mathsf{B}}\mathbf{1}_F \otimes \mathbf{L}_{-B} \phi} =\bpzbraket{\mathbf{\Psi}_{-\mathsf{A}}\mathbf{1}_F}{\mathsf{\Psi}_{-\mathbf{B}}\mathbf{1}_F} \times \bpzbraket{\mathbf{L}_{-A} \phi }{\mathbf{L}_{-B} \phi}.
\fe
where we use $A+B = 0~\text{mod}2$ for the matrix element to be non-vanishing. 

This parity operator is useful because the inner produce of Virasoro generator would factorize with this operator insertion
\ie
\bpzbraket{\mathbf{L}^{(1)}_{-M} \nu_1\otimes \mathbf{L}^{(2)}_{-N}  \nu_2} {(-1)^{p_{\mathsf{SCA}} \cdot p_{\mathsf{F}}} \mathbf{L}^{(1)}_{-M'} \otimes \nu_1\mathbf{L}^{(2)}_{-N'} \otimes \nu_2} = \bpzbraket{\mathbf{L}^{(1)}_{-M}  \nu_1} {\mathbf{L}^{(1)}_{-M'} \nu_1}\bpzbraket{\mathbf{L}^{(2)}_{-N}  \nu_2} {\mathbf{L}^{(2)}_{-N'} \nu_2}.
\fe
...
\todo[inline]{Yutai: I would actually expect that $P^{\dagger}=P$ under BPZ pairing.}
...
\ie
B_a&(h_1, h_2, h_3;n_1, n_2, n_3) = \frac{(-i)^a}{\prod_{i=1}^3\sqrt{\ell(2P_i, 4n_i)\ell(Q+2P_i, 4n_i)}}\\
&\quad \times \ell\left(\tfrac{Q}{2}+P_1+P_2+P_3, 2(n_1+n_2+n_3)\right)\ell\left(\tfrac{Q}{2}-P_1+P_2+P_3, 2(-n_1+n_2+n_3)\right)\\
&\quad \times \ell\left(\tfrac{Q}{2}+P_1+P_2+P_3, 2(n_1+n_2+n_3)\right) \ell\left(\tfrac{Q}{2}+P_1+P_2+P_3, 2(n_1+n_2+n_3)\right).
\fe
...
The earlier coefficientwise mismatch in this comparison is resolved.  It was caused by replacing the theta-polarized convolution $\star$ in (7.30) by ordinary formal-series multiplication when removing the auxiliary Majorana block.  With the $\star$-inverse used in (7.33), the direct PBW block, the $c$-recursive block, and the double-Virasoro block agree in all 20 symbolic coefficients through total physical level $3/2$.  The complete exact-rational level-two check gives 35 agreements out of 35 coefficients at each of three generic samples, hence 105 out of 105 agreements and zero mismatches.  The former counts $14/20$ and $78/105$ are retained only as a negative control for the obsolete ordinary-quotient prescription; they are not failures of the conformal-block decomposition.
...
\widehat{\mathbb{F}}^{(\eta, \eta')}_f&(h_1, \beta_2, \beta_3, c) = \sum_{\substack{
  |A|=|A'|, |B|=|B'|, |C|=|C'|\\
  |D|=|D'|, |E|=|E'|, |F|=|F'|\\
  n_1, n_2, n_3\in \frac{1}{2}\mathbb{Z}+\frac{1}{4}\\
...
and therefore,
\ie
\widehat{\mathbb{F}}_f^{(\eta,\eta')}(h_1, \beta_2, \beta_3, c) &= \sum_{\substack{n_1, n_2, n_3\in \frac{1}{2}\mathbb{Z}+\frac{1}{4}\\(-1)^{2n_1+\alpha_2+\alpha_3}=(-1)^f}} q_1^{2n_1^2}q_2^{2n_2^2-\frac{1}{8}}q_3^{2n_3^2-\frac{1}{8}}\eta_1^{2n_1}\eta_2^{2n_2}\eta_3^{2n_3}\\
